...
/**
 * agent-inbox.test.mjs — regression tests for agent-inbox.mjs.
 *
...
const CLI = join(ROOT, 'scripts/agent/agent-inbox.mjs');
...
function run(inbox, args, opts = {}) {
  return execFileSync(NODE, [CLI, ...args], {
    cwd: ROOT,
    env: { ...process.env, CAREER_ONE_INBOX: inbox },
    encoding: 'utf8',
...
console.log('6. first add on the default path self-heals .gitignore (idempotent)');
{
  ...
  const addOnce = () => execFileSync(NODE, [CLI, 'add', 'queue a scan'], {
    cwd: repo, env: { ...process.env, CAREER_ONE_INBOX: '' }, stdio: ['pipe', 'pipe', 'pipe'],
  });
  addOnce(); addOnce();
