...
/**
 * columns.test.mjs — regression tests for header-name column mapping.
 *
 ...
 * CAREER_ONE_TRACKER / CAREER_ONE_ADDITIONS env overrides and assert:
...
const SCRIPT_PATHS = {
  'dedup-tracker.mjs': 'scripts/tracker/dedup-tracker.mjs',
  'merge-tracker.mjs': 'scripts/tracker/merge-tracker.mjs',
  'tracker.mjs': 'scripts/tracker/tracker.mjs',
  'verify-pipeline.mjs': 'scripts/system/verify-pipeline.mjs',
};
...
function runScript(script, args, sandbox) {
  const env = {
    ...process.env,
    CAREER_ONE_TRACKER: sandbox.tracker,
    CAREER_ONE_ADDITIONS: sandbox.additions,
    CAREER_ONE_TRACKER_LOCK: sandbox.lock,
  };
  try {
    const stdout = execFileSync(NODE, [join(ROOT, SCRIPT_PATHS[script] || script), ...args], {
...
{
  const { removeRowByNum } = await import('../../scripts/tracker/tracker.mjs');
...
{
  const { loadSeenCompanyRoles } = await import('../../scripts/scan/scan.mjs');
  const sb = makeSandbox(HEADER_10);
...
// web/src/lib/tracker-table.mjs (behind readApplications() in career-one.ts)
// loads config/tracker-aliases.json — the same file tracker-parse.mjs exports as
...
if (!HAS_WEB) {
  ...
  const { HEADER_ALIASES } = await import('../../scripts/tracker/tracker-parse.mjs');
...
{
  const { resolveColumns, parseTrackerRow } = await import('../../scripts/tracker/tracker-parse.mjs');
...
if (!HAS_WEB) {
  ...
  const aliasFile = join(dir, 'config', 'tracker-aliases.json');
