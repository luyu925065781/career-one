...
/**
 * set-status.test.mjs — regression tests for the set-status.mjs CLI (#1428).
 *
 ...
 *
 * Tests provision a throwaway tracker via the CAREER_ONE_TRACKER /
 * CAREER_ONE_TRACKER_LOCK env overrides (same sandbox pattern as
...
import { acquireTrackerLock } from '../../scripts/tracker/tracker-utils.mjs';
...
function runSetStatus(args, sandbox, extraEnv = {}) {
  ...
  try {
    const stdout = execFileSync(NODE, [join(ROOT, 'scripts/tracker/set-status.mjs'), ...args], {
...
function makeSandbox(trackerContent) {
  ...
  // The lock env value must live under tmpdir and use the career-one prefix
  // (see trackerLockDirFor) or it is ignored — which would still be safe,
  // just contending on the real default lock.
  const lock = join(dir, 'career-one-merge-tracker-test.lock');
...
// ── 14. Lock timeout: structured exit 4, JSON error, no write ──
{
  ...
  const r = runSetStatus(['2', 'Applied', '--json'], sb, { CAREER_ONE_TRACKER_LOCK_TIMEOUT_MS: '300' });
  let parsed = null;
  try { parsed = JSON.parse(r.stdout); } catch {}
...
// ── 14b. Non-timeout lock failure is NOT reported as lock-timeout ─
{
  ...
  // timeouts.
  const blocker = join(sb.dir, 'career-one-merge-tracker-blocker');
  writeFileSync(blocker, 'not a directory');
  const badLock = join(blocker, 'career-one-merge-tracker-bad.lock');
...
// ── 15. Orphaned recovery guard does not block stale-lock recovery ─
{
  ...
  const lockDir = join(dir, 'career-one-merge-tracker-guardtest.lock');
...
// ── 16. Write failure surfaces as a structured error, not a stack ─
{
  if (process.platform !== 'win32' && process.getuid?.() === 0) {
    ...
    const lock = join(dir, 'career-one-merge-tracker-wf.lock');
