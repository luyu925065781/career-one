...
/**
 * followup-seed.test.mjs — regression tests for followup-seed.mjs (#1430).
 *
 ...
 *
 * Run: node test-all.mjs --only tracker/followup-seed
 */
...
import { parseNextOverrides, resolveNextOverride, normalizeStatus, addDays, parseDate } from '../../scripts/analysis/followup-cadence.mjs';

const ROOT = dirname(dirname(dirname(fileURLToPath(import.meta.url))));
const NODE = process.execPath;
const SCRIPT = join(ROOT, 'scripts/tracker/followup-seed.mjs');
...
function makeSandbox() {
  ...
  const lock = join(dir, `career-one-followups-test-${Math.random().toString(36).slice(2)}.lock`);
...
function run(args, sandbox, extraEnv = {}) {
  const env = {
    ...
    CAREER_ONE_FOLLOWUPS: sandbox.followups,
    CAREER_ONE_FOLLOWUPS_LOCK: sandbox.lock,
    ...extraEnv,
  };
  try {
...
// ── Test 9: lock held by a live pid → exit 4 ────────────────────────────────
{
  ...
  const res = run(['1'], sb, { CAREER_ONE_FOLLOWUPS_LOCK_TIMEOUT_MS: '200' });
