...
type PDFManifestEntry struct {
	ReportNumber string
	PDFPath      string // relative to the career-one root
	HTMLPath     string // relative to the career-one root, "" if unknown
...
func LoadPDFEntriesByPath(careerOnePath string) map[string]PDFManifestEntry {
	byPath := make(map[string]PDFManifestEntry)
	raw, err := os.ReadFile(filepath.Join(careerOnePath, "data", "pdf-index.tsv"))
	if err != nil {
		return byPath
	}
...
// LoadPDFManifest reads data/pdf-index.tsv under careerOnePath. A missing
...
func LoadPDFManifest(careerOnePath string) PDFManifest {
...
// ResolvePDFs returns candidate PDF paths (relative to careerOnePath) for an
...
func ResolvePDFs(careerOnePath string, app model.CareerApplication, manifest PDFManifest) []string {
	if entry, ok := manifest.Lookup(app); ok {
		if _, err := os.Stat(filepath.Join(careerOnePath, filepath.FromSlash(entry.PDFPath))); err == nil {
	...
	globbed, err := filepath.Glob(filepath.Join(careerOnePath, "output", "cv-*.pdf"))
	if err != nil {
		return nil
	}
	...
	for _, p := range globbed {
		...
		if matchesCompanySlug(base, slug) {
			if rel, err := filepath.Rel(careerOnePath, p); err == nil {
				matches = append(matches, filepath.ToSlash(rel))
			}
		}
	}

	sortPDFsNewestFirst(careerOnePath, matches)
	return matches
}
...
func sortPDFsNewestFirst(careerOnePath string, paths []string) {
	...
	mtimeOf := func(p string) int64 {
		info, err := os.Stat(filepath.Join(careerOnePath, filepath.FromSlash(p)))
		if err != nil {
			return 0
		}
...
func ResolveHTML(careerOnePath string, app model.CareerApplication) (htmlPath, pdfPath string) {
	slug := kebabCase(app.Company)
	if slug == "" {
		return "", ""
	}
	globbed, err := filepath.Glob(filepath.Join(careerOnePath, "output", "cv-*.html"))
	if err != nil {
		return "", ""
	}
	var matches []string
