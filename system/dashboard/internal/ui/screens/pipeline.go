...
import (
	...
	"github.com/luyu925065781/career-one/dashboard/internal/data"
	"github.com/luyu925065781/career-one/dashboard/internal/model"
	"github.com/luyu925065781/career-one/dashboard/internal/theme"
)
...
// CareerOnePath (as recorded in the manifest).
type PipelineGeneratePDFMsg struct {
	CareerOnePath string
	ReportNumber  string
	HTMLPath      string
	PDFPath       string
	Format        string
...
const storyTemplateURL = "https://github.com/luyu925065781/career-one/issues/new?template=i-got-hired.yml"
...
func NewPipelineModel(t theme.Theme, apps []model.CareerApplication, metrics model.PipelineMetrics, careerOnePath string, width, height int) PipelineModel {
...
func (m PipelineModel) WithReloadedData(apps []model.CareerApplication, metrics model.PipelineMetrics) PipelineModel {
	...
	reloaded := NewPipelineModel(m.theme, apps, metrics, m.careerOnePath, m.width, m.height)
...
func (m PipelineModel) handleKey(msg tea.KeyMsg) (PipelineModel, tea.Cmd) {
	...
	case "enter":
		if app, ok := m.CurrentApp(); ok && app.ReportPath != "" {
			fullPath := filepath.Join(m.careerOnePath, app.ReportPath)
	...
	case "d":
		if app, ok := m.CurrentApp(); ok {
			manifest := data.LoadPDFManifest(m.careerOnePath)
			candidates := data.ResolvePDFs(m.careerOnePath, app, manifest)
			if len(candidates) == 0 {
				m.flash = "No CV PDF found for this application — generate one with /career-one pdf"
			} else {
	...
	case "D":
		if app, ok := m.CurrentApp(); ok {
			...
			if !found || entry.HTMLPath == "" {
				byPath := data.LoadPDFEntriesByPath(m.careerOnePath)
				candidates := data.ResolvePDFs(m.careerOnePath, app, manifest)
			...
			if !found || entry.HTMLPath == "" {
				m.flash = "No source HTML found for this application — run /career-one pdf first"
				return m, nil
			}
			if _, err := os.Stat(filepath.Join(m.careerOnePath, filepath.FromSlash(entry.HTMLPath))); err != nil {
				m.flash = "Source HTML missing: " + entry.HTMLPath
				return m, nil
			}
			m.flash = "Regenerating PDF via generate-pdf.mjs — this takes a few seconds..."
			path, report := m.careerOnePath, entry.ReportNumber
...
func (m PipelineModel) StartDiscardReasonFlow(app model.CareerApplication, status string) (PipelineModel, tea.Cmd) {
	...
	reasons := data.LoadReportDiscardReasons(m.careerOnePath, app.ReportPath)
...
func (m PipelineModel) handleHiredFlow(msg tea.KeyMsg) (PipelineModel, tea.Cmd) {
	...
	case 3: // anonymous stat
		...
		case "y", "Y":
			...
			if err := data.SaveAnonymousStat(m.careerOnePath, m.hiredApp.Role, weeks); err != nil {
...
func (m PipelineModel) openPDFCmd(relPath string) tea.Cmd {
	fullPath := filepath.Join(m.careerOnePath, filepath.FromSlash(relPath))
	return func() tea.Msg {
...
func (m PipelineModel) loadCurrentReport() tea.Cmd {
	...
	path := m.careerOnePath
	report := app.ReportPath
	return func() tea.Msg {
		return PipelineLoadReportMsg{CareerOnePath: path, ReportPath: report}
	}
}
...
func (m PipelineModel) renderHelp() string {
	...
	brand := lipgloss.NewStyle().Foreground(m.theme.Overlay).Render("career-one by luyu925065781.io")
