...
`career-one` is a collection of Markdown prompts, Node.js scripts, and HTML templates. It is strictly a local execution tool. The maintainers do not host, deploy, or operate an AI system, nor do we provide API access to Large Language Models (LLMs).
...
career-one interacts with career portals and job boards (Greenhouse, Lever, Ashby, Workday, and others).
...
- The maintainers actively reject contributions that facilitate ToS violations (see [CONTRIBUTING.md](../.github/CONTRIBUTING.md)).

## 5. Acceptable Use

career-one is designed to help individuals make better career decisions, not to automate away human judgment. Acceptable use includes:
