...
Plugins extend career-one with integrations that need an API key or talk to an
...
> extend career-one itself.

## Using plugins

```bash
node career-one.mjs plugins list          # what's installed + its trust badge
node career-one.mjs plugins available     # bundled + community plugins we've approved
node career-one.mjs plugins add <name>    # install an approved community plugin
node career-one.mjs plugins enable <id>   # show the capability card (then add --confirm)
node career-one.mjs plugins skill <id>    # print a plugin's how-to (if it ships one)
```
...
If a plugin's files change without a version bump, career-one **blocks it** and
asks you to review + `node career-one.mjs plugins trust <id>` to re-pin (tamper detection).

## Writing a plugin

```bash
node career-one.mjs plugins new my-plugin     # scaffolds plugins.local/my-plugin/
```
...
1. Develop locally, then publish your plugin as its **own public GitHub repo**
   named exactly `career-one-plugin-<name>` (the template repo gives you the
...
3. Open a **registry PR** (the `?template=plugin-registry.md` template — the
   ...
   adds your `system/plugins-registry/<id>.json` file, pinned to an exact commit. CI
   ...
   `node career-one.mjs plugins add <name>` and your plugin ships to them via the normal
   update.
...
1. Publish `career-one-plugin-<id>` with the **same `id`** as the bundled plugin
...
3. Once approved + pinned, anyone who runs `node career-one.mjs plugins add career-one-plugin-<id>`
   installs your version, and the engine gives **your maintained successor
   precedence over the bundled reference** of the same id. `node career-one.mjs plugins available`
   surfaces the link: *"gmail — 🔁 maintained version: career-one-plugin-gmail"*.
...
code on your own machine — career-one is local-first and the source is yours; if
...
| --- | --- | --- | --- | --- |
| [career-one-plugin-tavily](https://github.com/Schlaflied/career-one-plugin-tavily) | Tavily search/extract for job scanning, liveness checks, and company research. | search | `TAVILY_API_KEY` | @Schlaflied |
| [career-one-plugin-google-calendar](https://github.com/Schlaflied/career-one-plugin-google-calendar) | Google Calendar ingest — detect upcoming interview events and surface them in the career-one pipeline. | ingest | `GOOGLE_CALENDAR_CLIENT_ID`, `GOOGLE_CALENDAR_CLIENT_SECRET`, `GOOGLE_CALENDAR_REFRESH_TOKEN` | @Schlaflied |
| [career-one-plugin-linkedin-alerts](https://github.com/Schlaflied/career-one-plugin-linkedin-alerts) | LinkedIn job alert ingest — parse LinkedIn alert emails from your Gmail inbox, normalize tracking links to canonical job URLs, and surface them in the career-one pipeline. | ingest | `GMAIL_CLIENT_ID`, `GMAIL_CLIENT_SECRET`, `GMAIL_REFRESH_TOKEN` | @Schlaflied |
| [career-one-plugin-outlook-interviews](https://github.com/Schlaflied/career-one-plugin-outlook-interviews) | Outlook interview ingest — detect interview invitation emails via Microsoft Graph, extract company / role / meeting link, and surface them in the career-one pipeline. | ingest | `MSGRAPH_CLIENT_ID`, `MSGRAPH_REFRESH_TOKEN` (optional: `MSGRAPH_CLIENT_SECRET`) | @Schlaflied |
| [career-one-plugin-obsidian](https://github.com/Schlaflied/career-one-plugin-obsidian) | Obsidian export — mirror the tracker into your vault as frontmatter notes queryable by Dataview/Bases; frontmatter belongs to the machine, the note body belongs to you. | export | None | @Schlaflied |
...
- **Centralized infrastructure** the project would run (hosted aggregation,
  ...
  [Discussion #904](https://github.com/luyu925065781/career-one/discussions/904).
- **Auto-submitting / blind-applying** → out of the core everywhere. career-one
