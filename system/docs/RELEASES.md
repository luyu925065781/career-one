...
`system/release.config.json` 是唯一可编辑的功能发布清单。`web/release.config.json` 是由 `release.mjs` 同步并校验的构建镜像，不要手工修改。每个功能只能处于：
...
`system/config/release-please-manifest.json` 记录已经发布的版本，不跟随每次开发版编号变化。
