...
/**
 * test-all.mjs — Comprehensive test suite for career-one
 *
...
const SOURCE_OVERRIDES = new Map([
  ...
  ['.claude/skills/career-one/SKILL.md', '.agents/skills/career-one/SKILL.md'],
  ['.opencode/skills/career-one/SKILL.md', '.agents/skills/career-one/SKILL.md'],
  ['.qwen/skills/career-one/SKILL.md', '.agents/skills/career-one/SKILL.md'],
  ['.antigravitycli/skills/career-one/SKILL.md', '.agents/skills/career-one/SKILL.md'],
  ['.grok/skills/career-one/SKILL.md', '.agents/skills/career-one/SKILL.md'],
]);

function sourcePath(relativePath) {
  const rootPath = join(ROOT, relativePath);
  if (existsSync(rootPath)) return rootPath;
  const override = SOURCE_OVERRIDES.get(relativePath);
  return override ? join(ROOT, override) : join(ROOT, 'system', relativePath);
}

/**
 * Read a repo-relative text file as UTF-8.
 *
 * @param {string} path - Path relative to the career-one repository root.
...
if (ONLY !== null) {
  ...
  console.log('\n🧪 career-one test suite (--only ' + ONLY + ')\n');
...
const systemMjsFiles = discoverMjsFiles(join(ROOT, 'system'));
for (const absolute of [...rootMjsFiles, ...implementationMjsFiles, ...systemMjsFiles]) {
...
const scripts = [
  ...
  { name: 'scripts/system/validate-portals.mjs --file system/templates/portals.example.yml', expectExit: 0 },
...
try {
  ...
  const insideHtmlPath = sourcePath('templates/cv-template.html');
  const outsideHtmlPath = join(dirname(ROOT), 'outside-cv-template.html');

  if (repoRelativeManifestPath(insideHtmlPath) === 'system/templates/cv-template.html') {
  ...
  if (
    ...
    injectedPageCss.indexOf('career-one-page-setup') < injectedPageCss.indexOf('</head>')
  ) {
  ...
  if (fragmentPageCss.startsWith('<style id="career-one-page-setup">')) {
...
if (
  /git\('diff',\s*'--name-only',\s*'HEAD',\s*'--',\s*'system\/dashboard'\)/.test(updateSystemScript) &&
  /path\.startsWith\(['"]system\/dashboard\/['"]\)\s*&&\s*path\.endsWith\(['"]\.go['"]\)/.test(updateSystemScript) &&
  /go build -o career-dashboard \./.test(updateSystemScript) &&
  /cwd:\s*join\(ROOT,\s*['"]system['"],\s*['"]dashboard['"]\)/.test(updateSystemScript) &&
...
if (updateSystemScript.includes("'system/'") && updateSystemScript.includes("'CODEX.md'")) {
  pass('update-system ships compact source and migrates the legacy CODEX.md wrapper');
} else {
  fail('update-system compact-source migration is incomplete');
}
...
for (const skillPath of ['.claude/skills/career-one/SKILL.md', '.agents/skills/career-one/SKILL.md']) {
  if (!existsSync(sourcePath(skillPath))) {
    fail(`${skillPath} is missing`);
    continue;
  }
  const skill = readFile(skillPath);
  if (skill.includes('/career-one latex') && skill.includes('`modes/latex.md`')) {
    pass(`${skillPath} exposes /career-one latex in discovery menu`);
  } else {
    fail(`${skillPath} does not expose /career-one latex in discovery menu`);
  }
  if (
    skill.includes('/career-one email') &&
    skill.includes('`modes/email.md`') &&
    skill.includes('只生成草稿，不发送')
  ) {
    pass(`${skillPath} exposes /career-one email in routing, discovery, and standalone loading`);
  } else {
    fail(`${skillPath} does not fully expose /career-one email`);
  }
}
...
const routerSkill = readFile('.agents/skills/career-one/SKILL.md');
if (
  /argument-hint:.*offer-prep/.test(routerSkill) &&
  routerSkill.includes('`modes/offer-prep.md`') &&
  routerSkill.includes('/career-one offer-prep') &&
  !/委派给隐藏的远程服务[\s\S]*offer-prep/.test(routerSkill)
) {
...
if (claudeMdDoc.includes('@AGENTS.md') && agentsMdDoc.includes('`offer-prep`')) {
...
try {
  const {
    ...
    SEED_SOURCES,
    SLUG_RE,
  } = await import(pathToFileURL(sourcePath('seeds/vc-portfolios.mjs')).href);
...
try {
  const tmp = mkdtempSync(join(tmpdir(), 'career-one-portals-validator-'));
...
`, 'utf-8');
  ...
  const exampleResult = run(NODE, ['scripts/system/validate-portals.mjs', '--file', 'system/templates/portals.example.yml']);
...
if (!existsSync(sourcePath('GEMINI.md'))) {
...
const codexWrapper = existsSync(sourcePath('CODEX.md')) ? readFile('CODEX.md') : '';
...
const canonicalSkill = '.agents/skills/career-one/SKILL.md';
const symlinks = [
  '.claude/skills/career-one/SKILL.md',
  '.opencode/skills/career-one/SKILL.md',
  '.qwen/skills/career-one/SKILL.md',
...
for (const link of symlinks) {
  let resolved = null;
  try {
    resolved = realpathSync(sourcePath(link));
    if (resolved !== canonicalReal) {
      const content = readFileSync(resolved, 'utf-8').trim();
      if (content.startsWith('..') && content.split('\n').length === 1) {
        resolved = realpathSync(join(dirname(sourcePath(link)), content));
      }
    }
  } catch {
    resolved = null;
...
if (
  ...
  /\/career-one/.test(canonicalContent ?? '')
) {
  pass('career-one skill router documents the Codex invocation model');
} else {
  fail('career-one skill router is missing Codex invocation guidance');
}
...
{
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'career-one-skills-'));
  try {
    const canonicalDir = join(fixtureRoot, '.agents', 'skills', 'career-one');
    const claudeDir = join(fixtureRoot, '.claude', 'skills', 'career-one');
    const opencodeDir = join(fixtureRoot, '.opencode', 'skills', 'career-one');
    mkdirSync(canonicalDir, { recursive: true });
    mkdirSync(claudeDir, { recursive: true });
    mkdirSync(opencodeDir, { recursive: true });

    const fixtureSkill = '---\nname: career-one\n---\n\n# canonical skill\n';
    const pointer = '../../../.agents/skills/career-one/SKILL.md';
...
{
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'career-one-ensure-skills-'));
  try {
    const canonicalDir = join(fixtureRoot, '.agents', 'skills', 'career-one');
    const claudeDir = join(fixtureRoot, '.claude', 'skills', 'career-one');
    mkdirSync(canonicalDir, { recursive: true });
    ...
    const skills = await import(pathToFileURL(sourcePath('scaffolder/bin/skill-entrypoints.mjs')).href);
    ...
    const grokSkill = readFileSync(join(fixtureRoot, '.grok', 'skills', 'career-one', 'SKILL.md'), 'utf-8');
    const claudeSkill = readFileSync(join(claudeDir, 'SKILL.md'), 'utf-8');
...

{
  ...
  try {
    const updater = await import(pathToFileURL(join(ROOT, 'update-system.mjs')).href);
    const sample = [
      "import { a } from './system/scaffolder/bin/skill-entrypoints.mjs';",
    ...
    const expected = [
      ...
      './sibling.mjs',
      './side-effect.mjs',
      './system/scaffolder/bin/skill-entrypoints.mjs',
    ];
    ...
    if (updater.relativeImportSpecifiers(liveSource).includes('./system/scaffolder/bin/skill-entrypoints.mjs')) {
...
{
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'career-one-skills-unreadable-'));
  try {
    const canonicalDir = join(fixtureRoot, '.agents', 'skills', 'career-one');
    const claudeDir = join(fixtureRoot, '.claude', 'skills', 'career-one');
    mkdirSync(canonicalDir, { recursive: true });
...
{
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'career-one-skills-entry-dir-'));
  try {
    const canonicalDir = join(fixtureRoot, '.agents', 'skills', 'career-one');
    const claudeDir = join(fixtureRoot, '.claude', 'skills', 'career-one');
    const opencodeDir = join(fixtureRoot, '.opencode', 'skills', 'career-one');
    ...
    const materialized = updater.materializeSkillEntrypoints(fixtureRoot);
    const opencodeSkill = readFileSync(join(opencodeDir, 'SKILL.md'), 'utf-8');
    if (JSON.stringify(materialized) === JSON.stringify(['.opencode/skills/career-one/SKILL.md']) && opencodeSkill === fixtureSkill) {
...
console.log('\n12c. Generated skill source boundary');

{
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'career-one-skill-git-'));
  ...
  try {
    ...
    writeFileSync(join(claudeDir, 'SKILL.md'), pointer);
    writeFileSync(join(opencodeDir, 'SKILL.md'), pointer);
    writeFileSync(join(fixtureRoot, '.gitignore'), '.claude/skills/career-one/\n.opencode/skills/career-one/\n');
    gitRun(['add', '--', '.agents/skills/career-one/SKILL.md', '.gitignore']);

    const pointerBlob = gitRun(['hash-object', '-w', '--stdin'], { input: pointer });
    gitRun(['update-index', '--add', '--cacheinfo', `120000,${pointerBlob},.claude/skills/career-one/SKILL.md`]);
    gitRun(['update-index', '--add', '--cacheinfo', `120000,${pointerBlob},.opencode/skills/career-one/SKILL.md`]);
    gitRun(['commit', '-m', 'fixture baseline']);

    const updater = await import(pathToFileURL(join(ROOT, 'update-system.mjs')).href);
    const materialized = updater.materializeSkillEntrypoints(fixtureRoot);
    updater.prepareMaterializedSkillEntrypointsForStage(materialized, fixtureRoot);

    const claudeIndex = gitRun(['ls-files', '-s', '--', '.claude/skills/career-one/SKILL.md']);
    const opencodeIndex = gitRun(['ls-files', '-s', '--', '.opencode/skills/career-one/SKILL.md']);
    if (!claudeIndex && !opencodeIndex) {
      pass('generated skill entrypoints are removed from the source index');
    } else {
      fail(`generated skill entrypoints remain tracked: ${JSON.stringify([claudeIndex, opencodeIndex])}`);
    }

    const claudeContent = readFileSync(join(claudeDir, 'SKILL.md'), 'utf8');
    const opencodeContent = readFileSync(join(opencodeDir, 'SKILL.md'), 'utf8');
    if (claudeContent === fixtureSkill && opencodeContent === fixtureSkill) {
      pass('generated skill files remain usable in the working tree');
    } else {
      fail('generated skill working-tree files no longer contain canonical content');
    }

    updater.hideGeneratedEntrypointsForCommit(materialized, fixtureRoot);
    gitRun(['commit', '-m', 'remove generated source entries', '--', ...materialized]);
    const committedGenerated = gitRun(['ls-tree', '-r', '--name-only', 'HEAD', '--', ...materialized]);
    if (!committedGenerated) {
      pass('generated skill entrypoint deletions survive path-scoped commits');
    } else {
      fail(`generated skill entrypoints were re-added by the scoped commit: ${committedGenerated}`);
    }

    updater.ensureSkillEntrypoints(fixtureRoot);
    if (
      readFileSync(join(claudeDir, 'SKILL.md'), 'utf8') === fixtureSkill
      && readFileSync(join(opencodeDir, 'SKILL.md'), 'utf8') === fixtureSkill
    ) {
      pass('generated skill files are restored after the source commit');
    } else {
      fail('generated skill files were not restored after the source commit');
    }
  } catch (e) {
...
try {
  ...
  const cliTmp = mkdtempSync(join(tmpdir(), 'career-one-add-cli-'));
  try {
...
try {
  ...
  const tmpDir = mkdtempSync(join(tmpdir(), 'career-one-migrate-'));
  try {
...
try {
  ...
  const reserveTmp = mkdtempSync(join(tmpdir(), 'career-one-reserve-'));
  const single = reserveRun([], reserveTmp);
  if (single === '001' && existsSync(join(reserveTmp, '001-RESERVED.md'))) {
    pass('CAREER_ONE_REPORTS_DIR override redirects sentinel to temp dir');
  } else {
  ...
  const rangeTmp = mkdtempSync(join(tmpdir(), 'career-one-reserve-range-'));
  ...
  const collideTmp = mkdtempSync(join(tmpdir(), 'career-one-reserve-collide-'));
  ...
  const rollbackTmp = mkdtempSync(join(tmpdir(), 'career-one-reserve-rollback-'));
  ...
  const concTmp = mkdtempSync(join(tmpdir(), 'career-one-reserve-conc-'));
  const spawnReserve = () => new Promise(resolve => {
    const child = spawn(NODE, [RESERVE, '--count', '4'], {
      env: { ...process.env, CAREER_ONE_REPORTS_DIR: concTmp },
    });
    let stdout = '';
  ...
  const relTmp = mkdtempSync(join(tmpdir(), 'career-one-reserve-release-'));
...
try {
  const vpTmp = mkdtempSync(join(tmpdir(), 'career-one-verify-reports-'));
  try {
    const vpReports = join(vpTmp, 'reports');
    mkdirSync(vpReports, { recursive: true });
    const vpTracker = join(vpTmp, 'applications.md');
    const vpEnv = { ...process.env, CAREER_ONE_TRACKER: vpTracker, CAREER_ONE_REPORTS: vpReports };
...
try {
  ...
  const dedupTmp = mkdtempSync(join(tmpdir(), 'career-one-dedup-'));
  try {
...
try {
  const rebuildTmp = mkdtempSync(join(tmpdir(), 'career-one-rebuild-'));
  try {
...
try {
  const locTmp = mkdtempSync(join(tmpdir(), 'career-one-dedup-loc-'));
  try {
...
try {
  const mergeTmp = mkdtempSync(join(tmpdir(), 'career-one-merge-'));
  try {
...
try {
  const viaTmp = mkdtempSync(join(tmpdir(), 'career-one-via-'));
  try {
...
try {
  ...
  const colTmp = mkdtempSync(join(tmpdir(), 'career-one-colorder-'));
  try {
...
try {
  const col912Tmp = mkdtempSync(join(tmpdir(), 'career-one-merge-912-'));
  try {
...
try {
  const reqTmp = mkdtempSync(join(tmpdir(), 'career-one-merge-1524-'));
  try {
...
try {
  const mergeTmp = mkdtempSync(join(tmpdir(), 'career-one-merge-lock-'));
  /**
  ...
  function spawnMerge(additionsDir, holdMs = 0) {
    ...
    const result = new Promise(resolve => {
      const child = spawn(NODE, ['scripts/tracker/merge-tracker.mjs'], {
        ...
        env: {
          ...process.env,
          CAREER_ONE_TRACKER: join(mergeTmp, 'data', 'applications.md'),
          CAREER_ONE_ADDITIONS: additionsDir,
          CAREER_ONE_TRACKER_LOCK: join(mergeTmp, 'career-one-merge-tracker-fixture.lock'),
          CAREER_ONE_MERGE_HOLD_MS: String(holdMs),
          CAREER_ONE_MERGE_READY_IPC: '1',
        },
...
try {
  ...
  const ac = JSON.parse(run(NODE, [sourcePath('doctor.mjs'), '--json', '--target', autoCopy]) || '{}');
  if (
...
if (!sqliteAvailable) {
  ...
  try {
    const idxTmp = mkdtempSync(join(tmpdir(), 'career-one-index-'));
    try {
...
try {
  ...
  const a = JSON.parse(run(NODE, [sourcePath('doctor.mjs'), '--json', '--target', noMcp]) || '{}');
  ...
  const b = JSON.parse(run(NODE, [sourcePath('doctor.mjs'), '--json', '--target', withMcp]) || '{}');
  ...
  const c = JSON.parse(run(NODE, [sourcePath('doctor.mjs'), '--json', '--target', withLocalMcp]) || '{}');
...
try {
  ...
  writeFileSync(join(batchDir, 'batch-runner.sh'), readFileSync(sourcePath('batch/batch-runner.sh'), 'utf-8').replace(/\r\n/g, '\n'));
...
try {
  const batchRunner = readFileSync(sourcePath('batch/batch-runner.sh'), 'utf-8');
  ...
  const codexArgsLine = batchRunner
    .split('\n')
    .find(l => l.includes('worker_args=(exec') && l.includes('--sandbox workspace-write') && l.includes('--search'));
  if (codexArgsLine) {
    pass('batch Codex workers run with workspace-write and search');
  } else {
    fail('batch Codex worker spawn missing workspace-write/search flags');
  }
} catch (e) {
...
try {
  ...
  if (parse('career-one-v1.9.0') === '1.9.0') {
    pass('SEMVER_RE parses Release Please component-prefixed tag (career-one-v1.9.0 → 1.9.0)');
  } else {
    fail(`SEMVER_RE failed on career-one-v1.9.0 (got ${parse('career-one-v1.9.0')}) — releases-API fallback is dead code (#923)`);
  }
  ...
  if (parse('career-one') === null && parse('v1.9') === null) {
    ...
    fail(`SEMVER_RE matched non-semver input (career-one → ${parse('career-one')}, v1.9 → ${parse('v1.9')})`);
  }
} catch (e) {
...
try {
  ...
  const fontFile = readdirSync(sourcePath('fonts')).find(f => f.endsWith('.woff2'));
...
try {
  ...
  const template = readFileSync(sourcePath('templates/cv-template.html'), 'utf-8');
...
try {
  ...
  for (const name of LIGATURE_TEMPLATES) {
    const css = readFileSync(sourcePath(`templates/${name}`), 'utf-8');
...
try {
  const cvTemplate = readFileSync(sourcePath('templates/cv-template.html'), 'utf-8');
  ...
  const resumeTemplate = readFileSync(sourcePath('templates/resume-template.html'), 'utf-8');
...
try {
  ...
  // CLAUDE.md delegates to AGENTS.md, while both surfaces keep the custom
  // extension point discoverable without duplicating the full rule set.
  const claudeMd = readFileSync(join(ROOT, 'CLAUDE.md'), 'utf-8');
  const agentsMd = readFileSync(join(ROOT, 'AGENTS.md'), 'utf-8');
  if (
    claudeMd.includes('@AGENTS.md') &&
    claudeMd.includes('modes/_custom.md') &&
    claudeMd.includes('modes/_custom.template.md') &&
...
try {
  ...
  const exampleYaml = readFileSync(sourcePath('templates/portals.example.yml'), 'utf-8');
...
try {
  ...
  const goodEntry = { name: 'career-one-plugin-x', id: 'x', repo: 'https://github.com/a/career-one-plugin-x', author: 'a', hooks: ['ingest'], requiredEnv: [], allowedHosts: ['api.x.com'], license: 'MIT', version: '1.0.0', sha: 'a'.repeat(40) };
  ...
  if (reg.validateRegistryEntry({ ...goodEntry, name: 'evil-x' }, regOpts).length > 0) pass('registry: name must start with career-one-plugin-');
  ...
  writeFileSync(join(succTmp, 'plugins-registry.json'), JSON.stringify({ registryVersion: 1, plugins: [{ name: 'career-one-plugin-gmail', id: 'gmail', repo: 'https://github.com/a/career-one-plugin-gmail', author: 'a', hooks: ['ingest'], requiredEnv: [], allowedHosts: [], license: 'MIT', version: '2.0.0', sha: SUCC_SHA, supersedesBundled: true }] }));
  ...
  if (reg.successorFor(succTmp, 'gmail')?.name === 'career-one-plugin-gmail') pass('successor: successorFor() surfaces the maintained version of a bundled id');
  else fail('successor: successorFor should return the registered successor');
  rmSync(succTmp, { recursive: true, force: true });

  if (install.parseRepoArg('alice/career-one-plugin-foo').id === 'foo') pass('install: owner/career-one-plugin-foo parses to id "foo"');
  ...
  if (nameRej) pass('install: refuses a repo not named career-one-plugin-*');
...
// Empty user directories are created by the distribution builder. Keeping
// placeholder files in source would put user-layer folders back in GitHub root.
const runtimePathsSource = readFile('distribution/runtime-paths.mjs');
if (runtimePathsSource.includes('"interview-prep/sessions"')) {
  pass('Distribution builder creates the interview session directory');
} else {
  fail('Distribution builder no longer creates the interview session directory');
}
...
// The compact source tree ships the builder, never user session contents.
{
  const updater = readFile('update-system.mjs');
  const sysBlock = (updater.match(/SYSTEM_PATHS\s*=\s*\[([\s\S]*?)\]/) || [, ''])[1];
  if (sysBlock.includes("'system/'")) pass('Distribution starter-directory source ships through SYSTEM_PATHS');
  else fail('Distribution starter-directory source is missing from SYSTEM_PATHS');
...
try {
  ...
  const statesSrc = readFileSync(sourcePath('templates/states.yml'), 'utf-8');
  ...
  const webParserPath = join(ROOT, 'web', 'src', 'lib', 'career-one.ts');
  if (existsSync(webParserPath)) {
    ...
    if (missingCols.length === 0) {
      pass('web/src/lib/career-one.ts still references the essential tracker columns');
    } else {
      fail(`web parser no longer references column(s): ${missingCols.join(', ')} — core and web drifted`);
    }
  } else {
    warn('web/src/lib/career-one.ts not found — web layer moved? update contract freeze section');
  }
} catch (e) {
...
try {
  ...
  if (
    titlesMode.includes('/career-one scan') &&
...
for (const skillPath of ['.claude/skills/career-one/SKILL.md', '.agents/skills/career-one/SKILL.md']) {
  if (!existsSync(sourcePath(skillPath))) continue; // existence already checked in section 8
  const skill = readFile(skillPath);
  if (
    /argument-hint:[^\n]*titles/.test(skill) &&
    skill.includes('`modes/titles.md`') &&
    skill.includes('/career-one titles') &&
    skill.includes('先展示配置差异并由用户确认')
  ) {
    pass(`${skillPath} exposes /career-one titles in argument-hint, routing, discovery, and standalone loading`);
  } else {
    fail(`${skillPath} does not fully expose /career-one titles`);
  }
}
...
try {
  ...
  if (claudeMdDoc.includes('@AGENTS.md') && agentsMdDoc.includes(titlesRow)) {
    pass('CLAUDE.md delegates the titles mode contract to AGENTS.md');
  } else {
...
try {
  const src = readFileSync(sourcePath('test-all.mjs'), 'utf-8');
