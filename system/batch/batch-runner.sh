...
# career-one batch runner — standalone orchestrator for headless AI workers
# Reads batch-input.tsv, delegates each offer to a Codex or Claude worker,
...
MIN_SCORE=0
SKIP_PDF=false
CLI="${CAREER_ONE_BATCH_CLI:-codex}"
MODEL=""  # Claude only: empty = let claude -p use the Claude Max default
...
usage() {
  cat <<'USAGE'
career-one batch runner — process job offers in batch via headless workers
Defaults to Codex (`codex exec --sandbox workspace-write --search`).
...
Options:
  ...
  --rate-limit-sleep N Seconds to wait before retrying a rate-limited worker
                       (default: 300)
  --model NAME         Claude-only model passed to `claude -p --model`.
                       Ignored for Codex.
...
check_prerequisites() {
  ...
  case "$CLI" in
    codex|claude) ;;
    *) echo "ERROR: --cli must be 'codex' or 'claude'."; exit 1 ;;
  esac
...
process_offer() {
  ...
  # Launch a headless worker. Codex reads the fully assembled worker prompt from
  # stdin and gets explicit workspace-write permission because the worker must
  # persist reports, PDFs, and tracker additions. Claude keeps its native
  # append-system-prompt-file path for backward compatibility.
  local -a worker_args
  local worker_bin="$CLI"
  local full_prompt=""
  if [[ "$CLI" == "claude" ]]; then
    ...
    if [[ -n "$MODEL" ]]; then
      echo "    ⚠️  --model is Claude-only; ignoring it for Codex."
    fi
    full_prompt="$BATCH_DIR/.codex-prompt-${id}.md"
    {
      cat "$resolved_prompt"
      printf '\n\n--- Batch task ---\n%s\n' "$prompt"
    } > "$full_prompt"
    worker_args=(exec --sandbox workspace-write --search -)
  fi
  ...
  while true; do
    exit_code=0
    if [[ "$CLI" == "claude" ]]; then
      "$worker_bin" "${worker_args[@]}" > "$log_file" 2>&1 || exit_code=$?
    else
      "$worker_bin" "${worker_args[@]}" < "$full_prompt" > "$log_file" 2>&1 || exit_code=$?
    fi
    ...
    if [[ "$CLI" == "claude" && $exit_code -eq 127 ]] && grep -qE "(claude: command not found|claude:.*not found|cannot find.*claude)" "$log_file" && (( shim_retries < max_shim_retries )); then
