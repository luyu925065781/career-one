...
> Claude Code marketplace metadata). These plugins extend career-one itself.
...
A plugin is a directory under `plugins/` (bundled, shipped with career-one) or
...
career-one is plain ESM with no build step, so the engine **cannot truly
...
- **Centralized infrastructure** the project would operate — hosted job
  ...
  [Where career-one is going (#904)](https://github.com/luyu925065781/career-one/discussions/904) —
  not the open-core.
- **Auto-submitting / blind-applying** to jobs. career-one is a decision-support
