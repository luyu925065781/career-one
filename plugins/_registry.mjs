...
/** Find a registry entry by bare id, full name, or `career-one-plugin-<id>`. */
export function findInRegistry(root, nameOrId) {
  const reg = loadRegistry(root);
  return reg.plugins.find(p =>
    p.id === nameOrId || p.name === nameOrId || p.name === `career-one-plugin-${nameOrId}`) || null;
}
...
export function validateRegistryEntry(e, { idRe, hookKinds, reservedEnv }) {
  const errs = [];
  if (typeof e.name !== 'string' || !e.name.startsWith('career-one-plugin-')) errs.push('name must start with "career-one-plugin-"');
  if (typeof e.id !== 'string' || !idRe.test(e.id)) errs.push('invalid id');
  if (e.name && e.id && e.name !== `career-one-plugin-${e.id}`) errs.push('name must equal career-one-plugin-<id>');
