...
// The China-mainland explorer exposes only sources the local scanner can read
// without platform access.
export type AtsSource = "official" | "greenhouse" | "lever" | "ashby" | "workday";
export const ATS_SOURCES: AtsSource[] = ["official"];
 ...
 *  buildTitleFilter / buildLocationFilter semantics. The current mainland
 *  source set contains only configured public target-company career pages. */
...
export type DiscoveredOffer = {
  ...
  title: string;
  location: string;
  /** YYYY-MM-DD, or "" when a public page does not expose a publish date. */
  postedAt: string;
  ats: string;
  source: string;
  ...
  /** present ONLY on AI offers → drives the "unverified" badge. AI finds cannot
   *  be liveness-confirmed; deterministic scan results come from a live public page. */
...
export type ScanEvent =
  ...
  | {
      ...
      unreachable: number;
      matches: number;
      // Authoritative degraded-vs-empty signals from the scanner's machine output.
