...
--- OUTPUT CONTRACT (the career-one WEB is parsing your stream) ---
...
export async function POST(req: Request) {
  ...
  let mode: string;
  try {
    mode = fs.readFileSync(path.join(careerOneRoot(), "modes", "discover.md"), "utf8");
  } catch {
    return Response.json({ code: "MODE_MISSING", error: "AI 搜索需要更新版择程AI，请先更新工具。" }, { status: 400 });
  }
  ...
  const args = isClaude
    ...
    : spec.args(prompt, { liveSearch: true });
