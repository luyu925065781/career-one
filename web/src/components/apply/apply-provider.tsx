...
export function ApplyProvider({ children }: { children: React.ReactNode }) {
  ...
  const drive = useCallback(async (id: string) => {
    ...
    try {
      ...
      if (!finished) {
        setError("Agent 在到达表单前停止了。");
        setStatus("error");
      }
    } catch (e) {
      setError(`Agent 无法到达表单：${e instanceof Error ? e.message : "响应流出错"}。`);
      setStatus("error");
    }
  }, []);
  ...
  const prefill = useCallback(async () => {
    ...
    if (!cliId()) {
      setError("请先在设置中配置 Agent CLI，再根据简历预填。");
      return;
    }
  ...
  const fill = useCallback(async () => {
    ...
    try {
      ...
      if (d.navigated) setError("填写过程中页面发生了跳转，请在提交前仔细核对。择程AI不会代替你提交申请。");
      setStatus("done");
  ...
  const agentFill = useCallback(async () => {
    ...
    try {
      ...
      setError(`Agent 无法填写表单：${e instanceof Error ? e.message : "响应流出错"}。`);
      setStatus("error");
    }
  }, []);
