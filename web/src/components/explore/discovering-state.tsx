...
.co-src__chip[data-state="active"]{border-color:color-mix(in srgb,var(--color-brand) 45%,transparent)}
...
.co-src__bar{height:3px;border-radius:2px;background:var(--gradient-primary);transition:width .4s ease}
  ...
  return (
    <div className="co-src__chip" data-state={state === "noisy" ? "active" : state}>
      {state === "active" ? (
        ...
        <Check className="size-3.5 text-icon-success" />
      ) : (
      ...
      <div className="ml-auto flex flex-col items-end gap-1">
        {state === "noisy" && <span className="text-[10px] text-faint">约 {s?.unreachable} 个跳过</span>}
