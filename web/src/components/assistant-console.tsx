...
const GREETING =
  "你好，我是择程AI求职助手。我可以帮你完善简历、评估岗位、查看求职进度或准备面试。你现在想先做什么？";
...
export function AssistantConsole() {
  ...
  const suggestions = useMemo(() => {
    ...
    if (rep) {
      chips.push({ label: "为什么是这个分数？", send: "请解释这个岗位得分的原因，包括优势和风险。" });
      chips.push({ label: "我该投递吗？", send: "结合我的个人配置，诚实判断我是否应该投递这个岗位。" });
      chips.push({ label: "起草求职信", send: "请为这个岗位起草一封简洁、有针对性的中文求职信。" });
      return chips;
    }
    const pending = pipeline.inbox.filter((j) => !j.done);
    if (!pipeline.applications.length && !pending.length) {
      return [
        { label: "完善我的资料", send: "请帮我完成择程AI（career-one）初始化。先检查已有文件，只询问缺失信息，并使用中文。" },
        { label: "优化我的简历", send: "查看我的简历，用中文提出最有价值的改进建议。" },
      ];
    }
    if (pending.length) {
      const counts = new Map<string, number>();
      ...
      if (top && top[1] > 1) chips.push({ label: `评估全部 ${top[0]}（${top[1]}）`, send: `评估岗位收件箱中全部待处理的 ${top[0]} 岗位。` });
      chips.push({ label: `筛选岗位收件箱（${pending.length}）`, send: `岗位收件箱中有 ${pending.length} 个岗位。请判断应优先评估哪些，并说明原因。` });
    }
    const strong = pipeline.applications.filter((a) => scoreNum(a.score) >= 4.5).length;
    if (strong) chips.push({ label: "处理高匹配岗位", send: "列出尚未投递且评分 4.5 以上的岗位，并告诉我优先顺序。" });
    chips.push({ label: "我今天应该做什么？", send: "查看我的求职进度，告诉我今天最值得做的三件事。" });
                ...
                <div key={i} className={cn("flex", m.role === "user" ? "justify-end" : "justify-start")}>
                  ...
                  >
                    {m.role === "user" ? (
                      ...
                      <Loader2 className="size-4 animate-spin text-icon-muted" />
                    ) : (
          ...
          <div className="border-t border-border p-3">
            <div className="flex items-end gap-2">
              <textarea
                ...
                placeholder={cliId ? "输入你的问题…" : "请先配置 Agent"}
                rows={1}
                disabled={!cliId}
    ...
    return (
      <div className="rounded-xl border border-border bg-surface/40 p-2.5">
        <div className="mb-1.5 flex items-center gap-1.5 text-xs font-medium">
          <Sparkles className="size-3.5 text-icon-brand" />
