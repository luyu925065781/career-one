...
import (
	...
	"github.com/luyu925065781/career-one/dashboard/internal/ui/screens"
)
...
func (m *appModel) reloadPipelineData() {
	apps := data.ParseApplications(m.careerOnePath)
...
func (m appModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case screens.PipelineLoadReportMsg:
		archetype, tldr, remote, comp := data.LoadReportSummary(msg.CareerOnePath, msg.ReportPath)
		m.pipeline.EnrichReport(msg.ReportPath, archetype, tldr, remote, comp)
		return m, nil

	case screens.PipelineUpdateStatusMsg:
		err := data.UpdateApplicationStatus(msg.CareerOnePath, msg.App, msg.NewStatus)
		if err != nil {
	...
	case screens.PipelineUpdateStatusAndNotesMsg:
		err := data.UpdateApplicationStatusAndNotes(msg.CareerOnePath, msg.App, msg.NewStatus, msg.NewNotes)
		if err != nil {
	...
	case screens.ViewerUpdateStatusMsg:
		...
		if normalized == "hired" {
			err := data.UpdateApplicationStatus(m.careerOnePath, msg.App, msg.NewStatus)
			if err != nil {
				fmt.Fprintf(os.Stderr, "WARN: status update failed: %v\n", err)
				m.reloadPipelineData()
				return m, nil
...
func runGeneratePDF(msg screens.PipelineGeneratePDFMsg) tea.Cmd {
	return func() tea.Msg {
		...
		cmd.Dir = msg.CareerOnePath
		out, err := cmd.CombinedOutput()
		if err != nil {
			return screens.PipelinePDFGeneratedMsg{Err: summarizeCmdError(err, out)}
		}
		pdfAbs := filepath.Join(msg.CareerOnePath, filepath.FromSlash(msg.PDFPath))
...
func main() {
	pathFlag := flag.String("path", ".", "Path to career-one directory")
	flag.Parse()

	careerOnePath := *pathFlag

	// Load applications
	apps := data.ParseApplications(careerOnePath)
	if apps == nil {
	...
	pm := screens.NewPipelineModel(t, apps, metrics, careerOnePath, 120, 40)

	for _, app := range apps {
		if app.ReportPath == "" {
			continue
		}
		archetype, tldr, remote, comp := data.LoadReportSummary(careerOnePath, app.ReportPath)
