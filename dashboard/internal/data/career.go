...
import (
	...
	"github.com/luyu925065781/career-one/dashboard/internal/model"
)
...
// relative to careerOnePath. Links are normally relative to the tracker
...
func resolveReportPath(careerOnePath, trackerPath, link string) string {
	resolved := filepath.Join(filepath.Dir(trackerPath), link)
	if _, err := os.Stat(resolved); err != nil {
		legacy := filepath.Join(careerOnePath, link)
		if _, err2 := os.Stat(legacy); err2 == nil {
			resolved = legacy
		}
	}
	if rel, err := filepath.Rel(careerOnePath, resolved); err == nil {
		return rel
	}
	return link
}
...
func ParseApplications(careerOnePath string) []model.CareerApplication {
	filePath := filepath.Join(careerOnePath, "applications.md")
	content, err := os.ReadFile(filePath)
	if err != nil {
		// Fallback: try data/ subdirectory
		filePath = filepath.Join(careerOnePath, "data", "applications.md")
		content, err = os.ReadFile(filePath)
		if err != nil {
			return nil
		}
	...
	for _, line := range lines {
		...
		// back to a careerOnePath-relative path, which is what every
		...
		if rm := reReportLink.FindStringSubmatch(at("report")); rm != nil {
			...
			app.ReportPath = resolveReportPath(careerOnePath, filePath, rm[2])
		}
	...
	batchURLs := loadBatchInputURLs(careerOnePath)
	reportNumURLs := loadJobURLs(careerOnePath)

	for i := range apps {
		if apps[i].ReportPath == "" {
			continue
		}
		fullReport := filepath.Join(careerOnePath, apps[i].ReportPath)
	...
	enrichFromScanHistory(careerOnePath, apps)

	// Strategy 5: company name fallback from batch-input.tsv
	enrichAppURLsByCompany(careerOnePath, apps)

	return apps
}

// loadBatchInputURLs reads batch-input.tsv and returns a map of batch ID -> job URL.
func loadBatchInputURLs(careerOnePath string) map[string]string {
	inputPath := filepath.Join(careerOnePath, "batch", "batch-input.tsv")
...
func loadJobURLs(careerOnePath string) map[string]string {
	...
	statePath := filepath.Join(careerOnePath, "batch", "batch-state.tsv")
...
func enrichFromScanHistory(careerOnePath string, apps []model.CareerApplication) {
	scanPath := filepath.Join(careerOnePath, "scan-history.tsv")
...
func enrichAppURLsByCompany(careerOnePath string, apps []model.CareerApplication) {
	inputPath := filepath.Join(careerOnePath, "batch", "batch-input.tsv")
	inputData, err := os.ReadFile(inputPath)
	if err != nil {
		return
...
// Aliases match states.yml -- keep in sync with career-one/states.yml
...
func LoadReportSummary(careerOnePath, reportPath string) (archetype, tldr, remote, comp string) {
	fullPath := filepath.Join(careerOnePath, reportPath)
...
func UpdateApplicationStatus(careerOnePath string, app model.CareerApplication, newStatus string) error {
	return UpdateApplicationStatusAndNotes(careerOnePath, app, newStatus, "")
}
...
func UpdateApplicationStatusAndNotes(careerOnePath string, app model.CareerApplication, newStatus string, newNotes string) error {
	filePath := filepath.Join(careerOnePath, "applications.md")
	content, err := os.ReadFile(filePath)
	if err != nil {
		filePath = filepath.Join(careerOnePath, "data", "applications.md")
...
func LoadReportDiscardReasons(careerOnePath, reportPath string) []string {
	if reportPath == "" {
		return nil
	}
	p := reportPath
	...
	fullPath := filepath.Join(careerOnePath, p)
	content, err := os.ReadFile(fullPath)
	if err != nil {
		return nil
	}
...
func SaveAnonymousStat(careerOnePath string, role string, weeks int) error {
	dirPath := filepath.Join(careerOnePath, "data")
