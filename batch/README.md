...
Batch mode reads offers from `batch-input.tsv`, but the `data/pipeline.md` inbox is a separate list. Without reconciliation, an offer evaluated by a batch run stays in the pipeline "Pendientes" section and gets surfaced again on the next scan or `/career-one pipeline` run -- producing duplicate reports.
...
`paused_rate_limit` is different: it means a worker hit a session, usage, or rate limit, so the runner stopped scheduling new offers and preserved the retry count. Resume those rows explicitly after the limit resets:
