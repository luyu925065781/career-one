...
图表色板直接使用不透明的 `accent-*` Token，不得混用 `icon-*`、`metric-*` 或状态 Token，也不得通过透明度临时拼色。评分分布固定为绿、黄、橙、红，对应从高到低的四档评分；多分类流程图使用不同色相区分类别；单序列排名图保持同一色相，只用长度编码数值，避免颜色暗示不存在的第二维度。
...
普通内容容器统一声明 `data-ui-card="quiet|subtle|solid"`。该合同适用于语义化的
`section`、`article`、`li`、整卡 `Link` 以及共享 `Card`，统一拥有 `card` 圆角、
无描边和对应表面层级；页面只保留内容布局、内边距和排版，不得重新拼装背景、描边或圆角。

普通内容卡片不使用描边，通过表面色、留白和必要时的轻微抬升建立层级。表格、输入框、
上传区、空状态、反馈容器和模态框仍按各自合同保留必要边界，不能被卡片规则误伤。

- `quiet`：与页面画布最接近，用于次级信息组和弱层级摘要。
- `subtle`：共享 `Card` 默认表面，用于普通内容卡与工作流模块。
- `solid`：完整 `surface`，用于信息密度较高或需要稳定阅读面的卡片。
- 只有整卡本身可点击时才声明 `data-card-interactive="true"`，由全局合同提供中性 Hover 和 Focus；静态卡片没有 Hover。
- 只有需要轻微抬升的统计块或重点卡片才声明 `data-card-elevation="raised"`；浮层和模态继续使用各自的 `floating` / `overlay` 层级，不伪装成普通 Card。

### 3.5 空状态

真正的无数据、无匹配结果或待配置状态统一声明 `data-ui-empty-state="panel|section"`。
该合同只拥有弱表面、虚线边界、圆角和默认文字色；页面继续决定图标、标题、说明、行动入口、
对齐和间距，避免把不同业务情境写成同一个模板。

- `panel`：替代卡片、表格或列表本应出现的内容区域，使用完整虚线边框、`card` 圆角和弱表面。
- `section`：替代页面中的整段内容，只使用上下虚线分隔，不额外制造一张卡片。
- 上传区、文件缩略图占位、运行状态和静态提示不属于空状态，不得只因使用虚线边框就接入该合同。
- 有明确下一步时，复用共享 `Button` 或按钮式链接；多个空状态同时出现时避免重复堆叠主按钮。
...
- 标准居中模态的遮罩声明 `data-ui-dialog-backdrop`，统一使用主题适配的 `dialog-backdrop`、`8px` 背景模糊、全屏定位和 `16px` 安全内边距；页面只保留 z-index。
- 标准内容面板声明 `data-ui-dialog="standard"`，统一使用 `panel` 圆角、语义描边、`surface` 背景和 `overlay` 阴影；页面继续拥有宽度、最大高度、滚动、内边距和内容布局。
- 属性合同只管理视觉外壳，不替代 `role="dialog|alertdialog"`、`aria-modal`、标题/说明关联、焦点捕获、Escape、焦点返回和背景关闭逻辑。
- 移动导航 Drawer、首次评分庆祝层和助手浮窗不是标准居中模态，继续使用各自专用结构。
...
浅色模式下每个指标色在白色背景上的对比度不低于 3:1；指标卡背景与描边必须成对复用同色相的
`*-surface` / `*-border` Token，品牌指标复用 `brand-soft` 与品牌细描边。不得把深色图标色低透明铺成
大面积背景，也不得二次稀释已有浅底 Token；指标卡顶部强调条直接继承数字的 `metric-*` 颜色，
不得再维护一套近似色。这样可让并排卡片保持一致的明度、饱和度和语义指向。
...
- 同时包含标题说明区与右侧搜索或动作区的顶级页面头部统一声明 `data-ui-page-header`，不得在页面重复拼装双列 flex。
- 桌面端采用 `minmax(0, 1fr) auto` 双列，允许标题说明区正确收缩，同时保持动作区的自然宽度。
- `639px` 及以下切换为单列，搜索与动作区获得完整可用宽度，标题不得被固定宽控件挤压成逐字换行。
- 页面继续拥有标题、说明、图标和动作内容；共享合同只负责顶级布局、间距和响应式切换。单列页面、卡片内部标题行、导航栏与模态头部不使用该合同。
