...
- **Listed** — one `plugins-registry/<id>.json` file; the code stays in the
  author's `career-one-plugin-<name>` repo; users `add` it. The default, low
  burden.
...
- **Naming + identity:** repo is `career-one-plugin-<name>`, `id` == name minus
...
- **Wording (public-forever):** description / README / skill carry no commercial
  / hosted-service / monetization language. career-one is free and local-first;
...
**not** registry-listed. It can still be a `career-one-plugin-<name>` repo users
...
feature PRs against it — close-redirect them to "publish `career-one-plugin-<id>`
