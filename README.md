...
首次运行时，Agent 会用中文引导你依次完成“简历 → 求职画像 → 面试故事库”，随后即可直接评估岗位，无需手工编辑配置文件。目标岗位、排除岗位和地点偏好统一保存在求职画像；`portals.yml` 仅在你需要招聘平台、目标公司招聘页或 ATS 等高级来源时按需创建。
...
Codex 不保证显示斜杠命令；交互使用时直接输入上述中文自然语言，一次性任务可运行 `codex exec "查看并总结当前求职进度。"`。
