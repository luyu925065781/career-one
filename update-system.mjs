...
/**
 * update-system.mjs — Safe auto-updater for career-one
 *
...
export const DEFAULT_GIT_TIMEOUT_MS = parsePositiveInt(process.env.CAREER_ONE_GIT_TIMEOUT_MS, 120000);
export const DEFAULT_GIT_FETCH_TIMEOUT_MS = parsePositiveInt(
  process.env.CAREER_ONE_GIT_FETCH_TIMEOUT_MS,
  Math.max(DEFAULT_GIT_TIMEOUT_MS, 300000),
);
export const NPM_INSTALL_TIMEOUT_MS = parsePositiveInt(process.env.CAREER_ONE_NPM_INSTALL_TIMEOUT_MS, 60000);
export const PLAYWRIGHT_INSTALL_TIMEOUT_MS = parsePositiveInt(process.env.CAREER_ONE_PLAYWRIGHT_INSTALL_TIMEOUT_MS, 120000);
export const DASHBOARD_REBUILD_TIMEOUT_MS = parsePositiveInt(process.env.CAREER_ONE_DASHBOARD_REBUILD_TIMEOUT_MS, 60000);
export const UPDATE_PATH_CHECKOUT_BUDGET_MS = parsePositiveInt(process.env.CAREER_ONE_UPDATE_PATH_CHECKOUT_BUDGET_MS, 5000);
export const REEXEC_BUFFER_TIMEOUT_MS = parsePositiveInt(process.env.CAREER_ONE_REEXEC_BUFFER_TIMEOUT_MS, 60000);
...
function gitTimeoutEnvVar(args) {
  return args[0] === 'fetch' ? 'CAREER_ONE_GIT_FETCH_TIMEOUT_MS' : 'CAREER_ONE_GIT_TIMEOUT_MS';
}
...
async function apply() {
  ...
  const isReexec = process.env.CAREER_ONE_UPDATE_REEXEC === '1';
  ...
  try {
    ...
    const backupBranch = process.env.CAREER_ONE_UPDATE_BACKUP_BRANCH || updateBackupBranchName(local);
    if (!isReexec) {
      try {
