...
Works in two modes:
- **Slug mode:** `/career-one cover {slug}` — loads the existing evaluation report draft as a starting point
- **Paste mode:** `/career-one cover` or JD pasted directly — starts from scratch
...
When invoked as `/career-one cover {slug}`:
