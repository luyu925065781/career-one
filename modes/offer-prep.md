...
1. `/career-one offer-prep {pasted contract text}`
2. `/career-one offer-prep {path to PDF or file}` — e.g. a contract dropped
   into `data/offers/{company-slug}/`
3. `/career-one offer-prep` — ask for the document
4. Proactively: when a tracker row is being set to `Offer`, suggest this mode.
5. `/career-one offer-prep reply {company-slug}` — Step 8 on demand: draft
