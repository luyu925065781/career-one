...
Обрабатывает URL вакансий из `data/pipeline.md`. Пользователь добавляет URL когда угодно, затем запускает `/career-one pipeline` для обработки.
...
2. **Для каждого URL**:
   a. Зарезервировать следующий `REPORT_NUM` атомарно, запустив `node career-one.mjs reserve-report-num` (и освободить маркер, запустив `node career-one.mjs reserve-report-num --release <num>` после записи отчета)
...
1. Запустить `node career-one.mjs reserve-report-num` для атомарного резервирования следующего порядкового номера (stdout вернет `{###}`).
2. Записать файл отчета, используя этот номер.
3. Освободить маркер, запустив `node career-one.mjs reserve-report-num --release {###}` после записи отчета.
