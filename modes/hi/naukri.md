...
- `{###}` = अगला sequential number (3 digits, zero-padded)। इसे atomically allocate करने के लिए `node career-one.mjs reserve-report-num` run करें (stdout `{###}` return करता है), report लिखें, फिर sentinel release करने के लिए `node career-one.mjs reserve-report-num --release {###}` run करें।
