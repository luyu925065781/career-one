...
Procesa las URLs de ofertas acumuladas en `data/pipeline.md`. El candidato añade URLs cuando quiere y luego lanza `/career-one pipeline` para procesarlas todas de una vez.

## Workflow

1. **Leer** `data/pipeline.md` -> encontrar los ítems `- [ ]` en la sección "Pendientes" / "Pending" / "En attente"
2. **Para cada URL pendiente**:
   a. Reservar el siguiente `REPORT_NUM` secuencial de forma atómica ejecutando `node career-one.mjs reserve-report-num` (y liberar el sentinel ejecutando `node career-one.mjs reserve-report-num --release <num>` una vez escrito el report)
...
1. Ejecutar `node career-one.mjs reserve-report-num` para reservar el siguiente número secuencial de forma atómica (stdout devuelve `{###}`).
2. Escribir el report con ese número.
3. Liberar el sentinel ejecutando `node career-one.mjs reserve-report-num --release {###}` una vez escrito el report.
