...
1. 使用规范 CLI 将该职位状态更新为 `Applied`：`node career-one.mjs set-status <report#> Applied`（不要手动编辑 `data/applications.md` 表格）。
2. 将最终实际提交的表单回答归档并更新至该报告的维度 H 模块中，以便后续面试对照。
3. 推荐下一步行动：引导候选人使用 `/career-one contacto` 指令生成 LinkedIn 招聘负责人直达勾兑话术。
