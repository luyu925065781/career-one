...
**故事库同步（必须执行）：** 每次完成维度 F 后读取 `interview-prep/story-bank.md`；文件不存在时创建。按真实经历和核心结果去重，不因同一 JD 的中英文报告重复添加。只写入能由 `cv.md`、`article-digest.md` 或用户确认信息支撑的事实；缺少数字、冲突、取舍或个人贡献边界时标记为“待完善”，不得补造。新故事使用 `Sxx` 编号，并记录能力标签、适用问题、事实来源、更新日期及 STAR+Reflection。
