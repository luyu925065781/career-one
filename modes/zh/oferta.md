...
当候选人输入职位描述（JD文本或URL链接）时，**始终**输出以下七个分析模块（A-F 分析 + G 真实性）。评分严格遵守 `_shared.md` 的 5 个评分因子；报告模块不是评分因子，不得因为新增内容而重复计分：
...
### 匹配雷达
使用 3-6 个与当前岗位真正相关的维度展示匹配程度。每个维度必须能由简历证据、岗位要求或明确的信息缺口解释，不得只根据关键词数量打分。

### 正向信号
从维度 B 的逐项匹配中提炼 3-6 条最有判断价值的正向信号。每条都要同时写清岗位要求、用户层文件中的对应证据，以及为什么这项证据能支持匹配；不得只复述关键词。
...
**故事库复用与候选沉淀：** 先读取 `interview-prep/story-bank.md`，优先匹配并复用已有故事。只有当 `cv.md`、`article-digest.md` 或用户确认信息中存在故事库尚未覆盖、且具有跨岗位复用价值的真实经历时，才生成候选故事；岗位特定的表达重点、追问和话术只保留在当前报告或 `interview-prep/{company}-{role}.md` 中。没有符合条件的新经历时，不创建或修改故事库。候选故事按真实经历与核心结果去重，同一 JD 的中英文报告不得生成重复候选；缺少数字、冲突、取舍或个人贡献边界时标记为“待完善”，不得补造。需要补充时，使用 `Sxx` 编号并记录能力标签、适用问题、事实来源、更新日期及 STAR+Reflection，先创建针对故事库的待确认提案，用户明确批准后才能写入。
...
### 剩余风险
汇总完成 A-G 分析后仍未消除的风险，区分候选人能力差距、证据不足、岗位信息缺口与公司/职位自身风险。不要重复维度 B 已经列出的补强动作。
...
用户界面按实际呈现顺序使用连续数字：1 岗位预览 → 2 简历匹配分析 → 3 级别判断与求职策略 → 4 薪酬竞争力与市场需求 → 5 职位真实性评估（含剩余风险）→ 6 打招呼话术 → 7 向招聘方追问 → 8 你在这个岗位里的最佳表达 → 9 沟通后分流规则 → 10 针对性定制方案 → 11 面试备考计划。
...
**PDF:** {CV生成状态，如 ❌ 或 ✅}
**Screenshots:** {截图任务提供的 data/task-attachments/... 相对路径，用 ` | ` 分隔；非截图输入时省略}
...
### 匹配雷达
(此处填入 3-6 个有证据支撑的岗位相关匹配维度)

### 正向信号
(此处填入“岗位要求 + 用户证据 + 匹配原因”的关键正向信号)

### 能力与缺口补强
(此处填入能力差距、证据缺口、可迁移经验与具体补强动作)
...
### 剩余风险
(此处汇总评估后仍未消除的候选人、岗位与公司风险)

## 打招呼话术
(生成一段只使用已确认事实的开场话术；不在标题中添加招聘平台名称)

## 向招聘方追问
(列出能改变是否申请、候选人定位或条件判断的关键问题；不要限定为猎头、HR 或具体招聘平台)

## 你在这个岗位里的最佳表达
(将岗位要求与用户已有证据连接成可直接用于沟通或面试的表达，不得创造新事实)

## 沟通后分流规则
(明确哪些新信息会让结论升分、维持或降分，并给出对应的投递、观察或放弃动作)
...
当任务指令提供本地截图路径时，必须逐张读取这些文件，并原样写入 `Screenshots` 头字段；不得复制到系统层或改写为绝对路径。Web 使用该字段在报告顶部展示原始岗位截图。
