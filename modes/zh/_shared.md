...
岗位评估使用 **5 个评分因子**计算 1-5 的综合匹配分。A-G 是报告的信息模块，不等同于评分因子；新增雷达、风险、沟通或备考内容时，不得因此重复计分。

| 评分因子 | 衡量内容 |
|---|---|
| 简历匹配度 (Match con CV) | 技能、经验与量化成果的匹配度 |
| 职业方向契合度 (North Star alignment) | 职位与候选人在 `_profile.md` 中定义的目标角色的契合程度 |
| 职级与职责匹配度 (Level and responsibility fit) | JD 的实际职级、职责范围、决策权、管理/协作边界与候选人自然职级的匹配程度 |
| 薪酬竞争力 (Comp) | 职位薪资与市场水准的对比 (5=顶尖, 1=远低于市场) |
| 组织与文化契合度 (Organization and culture fit) | 团队结构、价值观、工作方式、增长性、稳定性以及远程/考勤政策 |

**综合评分 (Global)** 是以上 5 个评分因子的加权平均分。风险、信息缺口和真实性信号必须在报告中显式呈现，但不能作为第六个因子再次扣分。

**分数解读：**
- 4.5+ → 极佳匹配；当 G 为高置信度时可建议立即投递，否则先按真实性评级处理
- 4.0-4.4 → 良好匹配，值得投递
...
## 综合判断 (Final Recommendation)

最终建议必须同时参考 **综合匹配分** 与 **职位真实性评级**：

- 综合匹配分回答“这个岗位是否适合候选人”。
- 职位真实性评级回答“这个招聘机会是否值得投入时间、是否需要先核实”。
- G 不修改数值分，但可以改变行动建议。例如高匹配分 + 谨慎推进，应建议“先核实再投递”；高匹配分 + 疑似虚假/已过期，应建议“优先核实或跳过”，不得仅凭高分建议立即投递。
...
| **996 / 弹性工作制** | 996 (朝九晚九周六工作)，或弹性上下班。 | 严重加班是组织与文化契合度的关键扣分项。需评估是否有加班费或调休，调休是否能落实。 |
| **税前/税后薪资** | 招聘时通常沟通税前月薪/年薪，但求职者更关注税后实际到手。 | 评分 Comp 时，默认以税前为准进行市场竞争力对比，但必须向求职者提供税后实得预估。 |
| **人才引进 / 落户政策** | 各地（如北京、上海、深圳）的落户政策、人才租房或税收补贴。 | 一些企业能提供落户指标或地方人才补贴，这是重要的加分项，需在组织与文化/附加福利中体现。 |
...
2. 在每个会话的第一次评估前运行 `node career-one.mjs sync-check`。如果发现同步警告，提示候选人。
