...
5. **career-one proof point:** includerlo nel campo "Informazioni aggiuntive" se presente nel modulo.
...
1. Aggiornare lo stato ad "Applied" con la CLI canonica: `node career-one.mjs set-status <report#> Applied` (non modificare la tabella di `applications.md` a mano).
2. Aggiornare il Blocco G del report con le risposte effettivamente inviate.
3. Suggerire il passo successivo: `/career-one contacto` per avviare il contatto su LinkedIn con il responsabile della selezione.
