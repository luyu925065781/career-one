...
Elabora gli URL degli annunci accumulati in `data/pipeline.md`. Il candidato aggiunge gli URL quando vuole, poi esegue `/career-one pipeline` per elaborarli tutti in una volta.
...
2. **Per ogni URL in attesa**:
   a. Riservare il prossimo `REPORT_NUM` sequenziale eseguendo `node career-one.mjs reserve-report-num` (liberare la prenotazione con `node career-one.mjs reserve-report-num --release <num>` una volta scritto il report)
...
1. Eseguire `node career-one.mjs reserve-report-num` per riservare in modo atomico il prossimo numero sequenziale (restituisce `{###}` su stdout).
2. Scrivere il report con questo numero.
3. Liberare la prenotazione con `node career-one.mjs reserve-report-num --release {###}` una volta scritto il report.
