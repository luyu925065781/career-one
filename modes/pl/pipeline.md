...
Przetwarza URL-e ofert nagromadzone w `data/pipeline.md`. Kandydat dodaje URL-e, kiedy chce, a potem uruchamia `/career-one pipeline`, by przetworzyć je wszystkie naraz.

## Workflow

1. **Przeczytaj** `data/pipeline.md` -> znajdź itemy `- [ ]` w sekcji "Oczekujące" / "Pending" / "Pendientes"
2. **Dla każdego oczekującego URL-a**:
   a. Zarezerwuj następny kolejny `REPORT_NUM` atomowo, uruchamiając `node career-one.mjs reserve-report-num` (i zwolnij sentinel, uruchamiając `node career-one.mjs reserve-report-num --release <num>`, gdy report zostanie zapisany)
...
1. Uruchom `node career-one.mjs reserve-report-num`, aby zarezerwować następny kolejny numer atomowo (stdout zwraca `{###}`).
2. Zapisz report z tym numerem.
3. Zwolnij sentinel, uruchamiając `node career-one.mjs reserve-report-num --release {###}`, gdy report zostanie zapisany.
