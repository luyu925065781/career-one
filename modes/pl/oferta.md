...
- `{###}` = następny kolejny numer (3 cyfry, dopełnione zerami). Aby przydzielić go atomowo i uniknąć race conditions, musisz uruchomić `node career-one.mjs reserve-report-num`, by zarezerwować numer (stdout zwraca `{###}`), zapisać report, a następnie uruchomić `node career-one.mjs reserve-report-num --release {###}`, by zwolnić sentinel.
