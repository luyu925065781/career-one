...
Обробляє URL вакансій з `data/pipeline.md`. Користувач додає URL коли завгодно, потім запускає `/career-one pipeline` для обробки.
...
2. **Для кожного URL**:
   a. Зарезервувати наступний `REPORT_NUM` атомарно, запустивши `node career-one.mjs reserve-report-num` (та звільнити маркер, запустивши `node career-one.mjs reserve-report-num --release <num>` після запису звіту)
...
1. Запустити `node career-one.mjs reserve-report-num` для атомарного резервування наступного порядкового номера (stdout поверне `{###}`).
2. Записати файл звіту, використовуючи цей номер.
3. Звільнити маркер, запустивши `node career-one.mjs reserve-report-num --release {###}` після запису звіту.
