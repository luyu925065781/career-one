...
**Банк історій:** Спочатку зіставляти й повторно використовувати наявні історії з `interview-prep/story-bank.md`. Пропонувати доповнення лише тоді, коли підтверджені факти користувача розкривають ще не записаний реальний досвід, придатний для різних ролей. Формулювання, питання й тези, специфічні для вакансії, залишаються в поточному звіті або `interview-prep/{company}-{role}.md`. Якщо відповідного нового досвіду немає, банк не змінювати й не створювати відсутній файл лише через завершення оцінювання. Будь-яке доповнення залишається пропозицією до явного схвалення користувача.
...
- `{###}` = наступний порядковий номер (3 цифри, zero-padded). Щоб виділити цей номер атомарно та уникнути станів гонки, ви ПОВИННІ виконати `node career-one.mjs reserve-report-num` для резервування номера (stdout поверне `{###}`), записати звіт, а потім виконати `node career-one.mjs reserve-report-num --release {###}` для звільнення маркера (sentinel).
...
Потім виконати `node career-one.mjs merge` для злиття в `data/applications.md`.
