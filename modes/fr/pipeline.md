...
Traite les URLs d'offres accumulees dans `data/pipeline.md`. Le candidat ajoute des URLs quand il veut et lance ensuite `/career-one pipeline` pour toutes les traiter d'un coup.

## Workflow

1. **Lire** `data/pipeline.md` -> trouver les items `- [ ]` dans la section "En attente" / "Pending" / "Pendientes"
2. **Pour chaque URL en attente** :
   a. Reserver le prochain `REPORT_NUM` sequentiel de maniere atomique en executant `node career-one.mjs reserve-report-num` (et liberer le sentinel en executant `node career-one.mjs reserve-report-num --release <num>` une fois le rapport ecrit)
...
1. Executer `node career-one.mjs reserve-report-num` pour reserver le prochain numero sequentiel de maniere atomique (stdout renvoie `{###}`).
2. Ecrire le rapport avec ce numero.
3. Liberer le sentinel en executant `node career-one.mjs reserve-report-num --release {###}` une fois le rapport ecrit.
