...
5. **career-one proof point** : inclure dans "Informations complementaires" si un tel champ existe
...
1. Mettre a jour le statut vers "Applied" via le CLI canonique : `node career-one.mjs set-status <report#> Applied` (ne pas editer la table `applications.md` a la main)
2. Mettre a jour le Bloc G du report avec les reponses finales
3. Suggerer l'etape suivante : `/career-one contacto` pour du LinkedIn outreach vers le hiring manager
