...
1. `/career-one email {report-number-or-slug}`
   ...
   - If `data/pdf-index.tsv` contains a PDF for that report, mention it as the CV
     ...
     generated first via `/career-one pdf {slug}` or attached manually.

2. `/career-one email {pasted JD}`
...
- `< 4.0`: restrained; do not oversell. If below 4.0, warn the user before
  drafting that career-one normally recommends against applying.
