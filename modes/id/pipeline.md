...
Memproses URL lowongan yang menumpuk di `data/pipeline.md`. Kandidat menambahkan URL kapan pun ia mau lalu menjalankan `/career-one pipeline` untuk memproses semuanya sekaligus.

## Alur kerja

1. **Baca** `data/pipeline.md` -> temukan item `- [ ]` di bagian "Menunggu" / "Pending" / "Pendientes"
2. **Untuk setiap URL yang menunggu**:
   a. Pesan `REPORT_NUM` urut berikutnya secara atomik dengan menjalankan `node career-one.mjs reserve-report-num` (dan lepas sentinel dengan menjalankan `node career-one.mjs reserve-report-num --release <num>` setelah report ditulis)
...
1. Jalankan `node career-one.mjs reserve-report-num` untuk memesan nomor urut berikutnya secara atomik (stdout mengembalikan `{###}`).
2. Tulis report dengan nomor tersebut.
3. Lepas sentinel dengan menjalankan `node career-one.mjs reserve-report-num --release {###}` setelah report ditulis.
