...
<!-- ============================================================
     ...
     that improve with each career-one release.
     ============================================================ -->

## Sources of Truth (EXCLUSIVE)

The files below are the **ONLY** sources for user-facing content (CV, cover letters, form answers, recruiter outreach). Auto-memory, parent-directory repos, and cross-session inferences are out of scope. See "Source-of-Truth Boundary" in `AGENTS.md` / `CLAUDE.md` / `system/compat/agents/CODEX.md` for the full rule.
...
1b. **First evaluation of each session:** Run `node career-one.mjs sync-check`. If warnings, notify user.
...
| Bash | `node career-one.mjs pdf` |

### Subagent delegation (cost guardrail)

A mode may tell you to run work in a background subagent (e.g. `scan`, or parallel `pipeline` URLs) to spare the main agent's context. Any subagent you spawn for career-one is a **single-pass worker**:
...
- One `/career-one <JD>` evaluates one role; it must never explode into a self-replicating swarm of agents. If you are about to delegate research or nest agents, stop and do it inline, bounded.
