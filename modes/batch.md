...
4. **For each pending URL**:
   ...
   c. Reserve the next REPORT_NUM atomically: `node career-one.mjs reserve-report-num` (release with `--release {num}` after the worker writes the report; stale sentinels are GC'd automatically)
   d. Execute via Bash:
...
```bash
node career-one.mjs reserve-report-num --count 8
...
```bash
node career-one.mjs reserve-report-num --release 042-049
```
...
`paused_rate_limit` means a worker hit a session, usage, or rate limit. The runner stops scheduling new offers, preserves the retry count, and resumes only when explicitly called with `--resume-paused`.
...
| Worker session/usage limit | Runner marks the current offer `paused_rate_limit`, stops scheduling new offers, preserves retries. Resume with `--resume-paused` after reset. |
