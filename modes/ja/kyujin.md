...
Report を保存し tracker に記録した後、report file に `## Cover Letter Draft` として cover letter draft を append する。これは starting point であり final letter ではない。ユーザーは `/career-one cover {slug}` で完成させる。
...
> Draft generated at evaluation time. Complete via `/career-one cover {slug}` to fill in angles, confirm research, and generate the PDF.
...
---
*Run `/career-one cover {slug}` to complete angles, confirm company research, and generate the PDF.*
```
...
- `{###}` = next sequential number（3 digits, zero-padded）。競合を防ぐため、必ず `node career-one.mjs reserve-report-num` を実行して番号を claim し（stdout returns `{###}`）、report を write してから `node career-one.mjs reserve-report-num --release {###}` を実行して sentinel を release する。
