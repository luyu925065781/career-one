...
- `{###}` = næste fortløbende nummer (3 cifre, nul-paddet). For at allokere det atomisk og undgå race conditions skal du køre `node career-one.mjs reserve-report-num` for at reservere nummeret (stdout returnerer `{###}`), skrive rapporten og derefter køre `node career-one.mjs reserve-report-num --release {###}` for at frigive sentinel'en.
