...
- `{###}` = sıradaki numara (3 haneli, sıfır dolgulu). Bu numarayı atomik olarak atamak ve yarış durumlarını (race condition) önlemek için, numarayı rezerve etmek üzere `node career-one.mjs reserve-report-num` komutunu çalıştırmalısınız (standart çıktı `{###}` değerini döndürür), raporu yazmalı ve ardından sentineli serbest bırakmak için `node career-one.mjs reserve-report-num --release {###}` komutunu çalıştırmalısınız.
...
Ardından `node career-one.mjs merge` çalıştır.
