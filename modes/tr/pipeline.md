...
`data/pipeline.md` dosyasına biriktirilen iş ilanı URL'lerini işler. İstediğin zaman URL ekle, hazır olduğunda `/career-one pipeline` komutunu çalıştır.

## İş Akışı

1. **Oku** `data/pipeline.md` → "Bekleyenler" bölümündeki `- [ ]` satırlarını bul
2. **Her bekleyen URL için:**
   a. Sıradaki `REPORT_NUM` değerini atomik olarak rezerve etmek üzere `node career-one.mjs reserve-report-num` komutunu çalıştır (ve rapor yazıldıktan sonra `node career-one.mjs reserve-report-num --release <num>` çalıştırarak sentineli serbest bırak)
...
1. Sıradaki rapor numarasını atomik olarak rezerve etmek için `node career-one.mjs reserve-report-num` komutunu çalıştır (standart çıktı `{###}` değerini döndürür).
2. Bu numarayı kullanarak rapor dosyasını yaz.
3. Rapor yazıldıktan sonra `node career-one.mjs reserve-report-num --release {###}` komutunu çalıştırarak sentineli serbest bırak.
