...
**Recommended Level 0:** run `node career-one.mjs scan` (or `npm run scan`) at the start of the agent's workflow. This covers local parsers + APIs in a single zero-token step and returns which companies used the `local-parser` successfully.
...
- Provider source of truth: `providers/`
...
3.5. **Level 0 — Local Parser** (`scan.mjs`, zero-token):
   ...
   Prefer running `node career-one.mjs scan` once to cover all zero-token local parsers + APIs; if executing manually, repeat the following logic.
...
→ Run the `pipeline` mode to evaluate the new offers (`/career-one pipeline` where available, or ask the agent to run `pipeline`).
```
