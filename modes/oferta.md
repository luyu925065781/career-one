...
If deeper company research is useful, recommend running `/career-one deep` separately after the evaluation.
...
After saving the report and recording in the tracker, append a cover letter draft to the report file under `## Cover Letter Draft`. This is a starting point — not the final letter. The user completes it via `/career-one cover {slug}`.
