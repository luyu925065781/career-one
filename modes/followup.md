...
```bash
node career-one.mjs followup-cadence
```
...
> "No active applications to follow up on. Apply to some roles first with `/career-one` and come back when they're aging."
...
- Suggest the user run `/career-one contacto {company}` to find the right person first
...
> - Trying a different contact via `/career-one contacto`
...
**To:** {email or "No contact found — run `/career-one contacto` first"}
...
`node career-one.mjs followup-seed <num>` (run by the `apply` mode's Step 9) appends a
...
These defaults can be overridden via `node career-one.mjs followup-cadence --applied-days N`.
