...
- `{###}` = próximo número sequencial (3 dígitos, zero-padded). Para alocar de forma atômica e evitar condições de corrida, você DEVE executar `node career-one.mjs reserve-report-num` para reservar o número (a saída retornará `{###}`), escrever o relatório, e em seguida executar `node career-one.mjs reserve-report-num --release {###}` para liberar o sentinel.
