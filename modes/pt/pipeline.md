...
Processa URLs de vagas acumuladas em `data/pipeline.md`. O candidato adiciona URLs quando quiser e depois executa `/career-one pipeline` para processar todas de uma vez.

## Workflow

1. **Ler** `data/pipeline.md` → buscar itens `- [ ]` na seção "Pendentes"
2. **Para cada URL pendente**:
   a. Reservar o próximo `REPORT_NUM` sequencial de forma atômica executando `node career-one.mjs reserve-report-num` (e liberar o sentinel executando `node career-one.mjs reserve-report-num --release <num>` após a gravação do relatório)
...
1. Executar `node career-one.mjs reserve-report-num` para reservar o próximo número sequencial de forma atômica (a saída retornará `{###}`).
2. Gravar o arquivo de relatório usando esse número.
3. Liberar o sentinel executando `node career-one.mjs reserve-report-num --release {###}` após a gravação do relatório.
