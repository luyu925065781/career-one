# Contexto Compartilhado -- career-one (Português BR)
...
1b. **Na primeira avaliação de cada sessão:** Executar `node career-one.mjs sync-check` via Bash. Se houver avisos, informar o candidato antes de continuar
...
| Bash | `node career-one.mjs pdf`, `node career-one.mjs merge` |
