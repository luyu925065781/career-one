...
5. **career-one proof point**: "추가 정보" 같은 필드가 있으면 포함
...
1. tracker status 변경은 `data/applications.md`를 직접 수정하지 않습니다. `batch/tracker-additions/`에 `Applied` 상태의 TSV update를 쓰고 `node career-one.mjs merge`로 병합합니다.
2. 최종 답변을 report의 블록 G에 업데이트합니다.
3. 다음 단계로 `/career-one contacto`를 제안해 hiring manager 또는 recruiter에게 LinkedIn outreach를 할 수 있게 합니다.
