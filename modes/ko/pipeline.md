...
`data/pipeline.md`에 쌓인 채용 공고 URL을 처리합니다. 후보자는 원할 때 URL을 추가하고, 이후 `/career-one pipeline`을 실행해 한 번에 처리합니다.

## Workflow

1. **읽기** `data/pipeline.md` -> "대기" / "Pending" / "Pendientes" 섹션의 `- [ ]` item 찾기
2. **각 대기 URL에 대해**:
   a. `node career-one.mjs reserve-report-num`를 실행해 다음 sequential `REPORT_NUM`을 atomic하게 예약합니다(report 작성 후 `node career-one.mjs reserve-report-num --release <num>`으로 sentinel 해제).
...
1. `node career-one.mjs reserve-report-num`를 실행해 다음 sequential number를 예약합니다(stdout이 `{###}` 반환).
2. 해당 번호로 report를 작성합니다.
3. report 작성 후 `node career-one.mjs reserve-report-num --release {###}`를 실행해 sentinel을 해제합니다.
