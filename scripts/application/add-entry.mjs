#!/usr/bin/env node
/**
 * add-entry.mjs — Deterministic dedup + insertion for `/career-one add`.
 *
 ...
 *
 * Usage:
 *   node career-one.mjs add <payload.json> [--dry-run]
 *   node career-one.mjs add --stdin [--dry-run]        (read payload JSON from stdin)
 *
 ...
 *
 * Test isolation: CAREER_ONE_CV / CAREER_ONE_ARTICLE_DIGEST override the target
...
const CV_FILE = process.env.CAREER_ONE_CV || join(CAREER_ONE, 'cv.md');
const ARTICLE_DIGEST_FILE = process.env.CAREER_ONE_ARTICLE_DIGEST || join(CAREER_ONE, 'article-digest.md');
...
export function applyAdd(payload, { cvText = null, articleText = null } = {}) {
  ...
  if (payload.articleDigest) {
    ...
    const current = articleText === null
      ? '# Article Digest -- Proof Points\n\nCompact proof points from portfolio projects. Read by career-one at evaluation time.\n'
      : articleText;
...
async function main() {
  ...
  if (!useStdin && !fileArg) {
    console.error('Usage: node career-one.mjs add <payload.json> [--dry-run]  (or --stdin)');
    process.exit(1);
  }
