 ...
 *
 * Usage:
 *   node career-one.mjs prepare-application --url <apply_url> --pdf output/<cv>.pdf
 *   node career-one.mjs prepare-application --url <apply_url> --pdf output/<cv>.pdf --cover cover.txt
 *
 * Supported ATS:
...
if (!applyUrl || !pdfPath) {
  console.error('Usage: node career-one.mjs prepare-application --url <apply_url> --pdf <pdf_path> [--cover <cover_txt>]');
  process.exit(1);
}
