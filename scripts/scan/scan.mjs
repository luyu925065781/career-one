 ...
 *
 * Usage:
 *   node career-one.mjs scan                  # scan all enabled companies
 *   node career-one.mjs scan --dry-run        # preview without writing files
 *   node career-one.mjs scan --company Cohere # scan a single company
 *   node career-one.mjs scan --verify         # Playwright-check each new URL; drop expired postings
 *   node career-one.mjs scan --verify --headed-fallback  # retry anti-bot-blocked URLs in a headed browser (needs a display)
 *   node career-one.mjs scan --verify --throttle          # jittered ~5-10s gap between checks (stay under rate limits)
 *   node career-one.mjs scan --verify --throttle=8000     # custom base gap in ms (waits base..2*base)
 */
...
import { classifyFetchError } from '../system/verify-portals.mjs';
...
const PORTALS_PATH = process.env.CAREER_ONE_PORTALS || 'portals.yml';
const PROFILE_PATH = process.env.CAREER_ONE_PROFILE || 'config/profile.yml';
const SCAN_HISTORY_PATH = 'data/scan-history.tsv';
const PIPELINE_PATH = 'data/pipeline.md';
const APPLICATIONS_PATH = 'data/applications.md';
const PROVIDERS_DIR = path.resolve(path.dirname(path.dirname(path.dirname(fileURLToPath(import.meta.url)))), 'providers');
...
// Public career pages do not always expose a publish date. Apply the selected
// freshness window when a provider supplies one, but keep undated postings so a
// sparse official site is not silently treated as empty.
export function buildPostedAfterFilter(sinceDays, nowMs = Date.now()) {
  const days = Number(sinceDays);
  if (!Number.isFinite(days) || days <= 0) return () => true;
  const cutoff = nowMs - Math.round(days) * 24 * 60 * 60 * 1000;
  return (postedAt) => postedAt == null || !Number.isFinite(Number(postedAt)) || Number(postedAt) >= cutoff;
}

export const WEB_SCAN_RESULT_PREFIX = '@@CAREER_ONE_SCAN_JSON@@';

export function formatWebScanResult(payload) {
  return WEB_SCAN_RESULT_PREFIX + JSON.stringify(payload);
}
...
// in modes/pipeline.md and expected by /career-one pipeline.
const PIPELINE_SKELETON = `# Pipeline — Pending URLs

Paste job URLs below as \`- [ ] {url}\` then run \`/career-one pipeline\`.
...
function pipelineUrlKey(value) {
  const raw = normalizeScanScalar(value);
  if (!raw) return '';
  try {
...
export function appendToPipeline(offers) {
  ...
  const seenUrls = new Set(
    [...text.matchAll(/- \[[ x]\] (https?:\/\/\S+)/g)]
      .map((match) => pipelineUrlKey(match[1]))
      .filter(Boolean),
  );
  const freshOffers = offers.filter((offer) => {
    const key = pipelineUrlKey(offer?.url);
  ...
  if (idx === -1) {
    ...
    const block = `\n## Pending\n\n` + freshOffers.map(formatPipelineOffer).join('\n') + '\n\n';
...
async function verifyOffers(offers, { headedFallback = false, throttleBaseMs = 0, rediscover = false } = {}) {
  ...
  try {
    ...
    ({ checkUrlLiveness, checkUrlLivenessWithFallback, createHeadedPageProvider, newLivenessPage, jitteredDelayMs, sleep } = await import('../liveness/liveness-browser.mjs'));
  } catch (err) {
    throw new Error(
...
async function main() {
  ...
  const sinceDays = sinceIndex !== -1 ? Number(args[sinceIndex + 1]) : 0;
  const postedAfterFilter = buildPostedAfterFilter(sinceDays);
  ...
  const tasks = targets.map(company => async () => {
    ...
    try {
      ...
      for (const job of jobs) {
        ...
        if (!postedAfterFilter(job.postedAt)) {
  ...
  if (sinceDays > 0) console.log(`Filtered by recency:  ${totalFilteredRecency} removed`);
  ...
  if (unreachableTargets.length > 0) {
    ...
    console.log(`\n⚠️  ${unreachableTargets.length} target(s) unreachable (slug?): ${names} — run: node career-one.mjs verify-portals`);
  }
  ...
  if (jsonOutput) {
    console.log(formatWebScanResult({
      companiesAvailable: companies.filter((company) => company && company.enabled !== false && company.scan_method !== 'websearch').length,
      companiesScanned: summaryCompanies,
      postingsKept: verifiedOffers.length,
      postingsUndated: verifiedOffers.filter((offer) => offer.postedAt == null).length,
      unreachableTargets: errors.length,
      offers: verifiedOffers.map((offer) => ({
        company: offer.company,
        title: offer.title,
        url: offer.url,
        location: offer.location || '',
        postedAt: Number.isFinite(Number(offer.postedAt)) ? new Date(Number(offer.postedAt)).toISOString().slice(0, 10) : '',
        source: offer.source,
      })),
    }));
  }
  ...
  console.log(`\n→ Run /career-one pipeline to evaluate new offers.`);
  console.log('→ Share results and get help: https://discord.gg/8pRpHETxa4');
}

// Only run main() when invoked directly (`node career-one.mjs scan`), not when imported by tests.
