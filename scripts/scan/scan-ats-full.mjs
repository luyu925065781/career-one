 ...
 *
 * Usage:
 *   node career-one.mjs scan-full                      # scan all ATS directories, last 3 days
 ...
 *   node career-one.mjs scan-full --limit 200          # max companies per ATS (default: all)
 *   node career-one.mjs scan-full --dry-run            # preview without writing files
 *   node career-one.mjs scan-full --liveness           # Playwright-verify matches before writing
 *   node career-one.mjs scan-full --verbose            # log per-board fetch failures
 *   node career-one.mjs scan-full --md-out <dir>       # also write a dated markdown digest to <dir>
 *   node career-one.mjs scan-full --help               # print this usage block and exit
 */
...
async function filterLive(offers) {
  ...
  try {
    ...
    ({ checkUrlLiveness, newLivenessPage } = await import('../liveness/liveness-browser.mjs'));
  } catch (err) {
    throw new Error(
