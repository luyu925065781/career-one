#!/usr/bin/env node
/**
 * detect-reposts.mjs — Repost Detector for career-one
 *
 ...
 *
 * Run: node career-one.mjs reposts             (JSON to stdout)
 *      node career-one.mjs reposts --summary   (human-readable table)
 *      node career-one.mjs reposts --window 60 (override 90-day window)
 *      node career-one.mjs reposts --self-test
 *
 * Issue #1205 — github.com/luyu925065781/career-one
 */
...
const SCAN_HISTORY_PATH = join(CAREER_ONE, 'data/scan-history.tsv');
...
function printSummary(clusters) {
  console.log(`\n${'='.repeat(78)}`);
  console.log('  Repost Detector — career-one');
