#!/usr/bin/env node
/**
 * process-quality.mjs — Recruiting-Process Friction Aggregator for career-one
 *
 ...
 *
 * Run: node career-one.mjs process-quality             (JSON to stdout)
 *      node career-one.mjs process-quality --summary   (human-readable table)
 *      node career-one.mjs process-quality --min-threshold 2  (min total interviews per company to report)
 *      node career-one.mjs process-quality --file path/to/active-interviews.md  (override the data path; test isolation)
 *      node career-one.mjs process-quality --self-test
 *
 * Issue #1466 — github.com/luyu925065781/career-one
 */
...
const DEFAULT_ACTIVE_INTERVIEWS_PATH = existsSync(join(CAREER_ONE, 'data/active-interviews.md'))
  ? join(CAREER_ONE, 'data/active-interviews.md')
  : join(CAREER_ONE, 'active-interviews.md');
...
function printSummary(signals) {
  console.log(`\n${'='.repeat(78)}`);
  console.log('  Process Quality Signal — career-one');
