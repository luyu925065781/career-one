#!/usr/bin/env node
/**
 * analyze-patterns.mjs — Rejection Pattern Detector for career-one
 *
 ...
 *
 * Run: node career-one.mjs patterns          (JSON to stdout)
 *      node career-one.mjs patterns --summary (human-readable table)
 *      node career-one.mjs patterns --min-threshold 3
 *      node career-one.mjs patterns --min-vendor-n 8   (per-vendor sample floor)
...
function analyze() {
  ...
  const enriched = entries.map(e => {
    ...
    const reportPath = reportMatch ? join(CAREER_ONE, reportMatch[1]) : null;
