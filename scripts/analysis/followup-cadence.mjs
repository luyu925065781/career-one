#!/usr/bin/env node
/**
 * followup-cadence.mjs — Follow-up Cadence Tracker for career-one
 *
 ...
 *
 * Run: node career-one.mjs followup-cadence             (JSON to stdout)
 *      node career-one.mjs followup-cadence --summary   (human-readable dashboard)
 *      node career-one.mjs followup-cadence --overdue-only
 *      node career-one.mjs followup-cadence --applied-days 10
 */
...
const FOLLOWUPS_FILE = join(CAREER_ONE, 'data/follow-ups.md');
const PROFILE_FILE = process.env.CAREER_ONE_PROFILE || join(CAREER_ONE, 'config/profile.yml');
...
// --- Resolve report path ---
export function resolveReportPath(reportField, appsFile = APPS_FILE, repoRoot = CAREER_ONE) {
