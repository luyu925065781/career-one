 ...
 *
 * Run: node career-one.mjs salary-gap             (JSON)
 *      node career-one.mjs salary-gap --summary   (human-readable)
...
const OBS_PATH = join(CAREER_ONE, 'data/salary-observations.tsv');
...
function loadProfileDesired() {
  const profilePath = join(CAREER_ONE, 'config/profile.yml');
