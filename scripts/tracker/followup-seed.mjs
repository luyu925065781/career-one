 ...
 *
 * Usage:
 *   node career-one.mjs followup-seed <appNum> [--date YYYY-MM-DD] [--force] [--dry-run] [--json]
 *   node career-one.mjs followup-seed --backfill [--dry-run] [--json]
 *
 * Exit codes:
 ...
 *   CAREER_ONE_TRACKER                     tracker path
 *   CAREER_ONE_FOLLOWUPS                   follow-ups path
 *   CAREER_ONE_PROFILE                     profile.yml path (cadence overrides)
 *   CAREER_ONE_FOLLOWUPS_LOCK              lock directory override
 *   CAREER_ONE_FOLLOWUPS_LOCK_TIMEOUT_MS   lock acquire timeout
 *   CAREER_ONE_FOLLOWUPS_LOCK_RETRY_MS     lock retry interval
 *   CAREER_ONE_FOLLOWUPS_LOCK_STALE_MS     stale-lock recovery threshold
 */
...
import {
  ...
  parseDate,
  addDays,
} from '../analysis/followup-cadence.mjs';
...
const FOLLOWUPS_LOCK_PREFIX = 'career-one-followups-';
...
function resolveTrackerPath(override) {
  ...
  if (process.env.CAREER_ONE_TRACKER) return process.env.CAREER_ONE_TRACKER;
  return existsSync(join(CAREER_ONE, 'data/applications.md'))
...
function resolveFollowupsPath(override) {
  if (override) return override;
  if (process.env.CAREER_ONE_FOLLOWUPS) return process.env.CAREER_ONE_FOLLOWUPS;
  return join(CAREER_ONE, 'data/follow-ups.md');
}
...
function resolveLockDir(explicitLockDir, followupsPath) {
  ...
  return resolveFollowupsLockDir(process.env.CAREER_ONE_FOLLOWUPS_LOCK, lockKey);
}
...
export async function seedFollowup(appNum, options = {}) {
  ...
  const lock = await acquireFollowupsLock(lockDir, followupsPath, {
    timeoutMs: options.lockTimeoutMs ?? envInt('CAREER_ONE_FOLLOWUPS_LOCK_TIMEOUT_MS', 60_000),
    retryMs: options.lockRetryMs ?? envInt('CAREER_ONE_FOLLOWUPS_LOCK_RETRY_MS', 75),
    staleMs: options.lockStaleMs ?? envInt('CAREER_ONE_FOLLOWUPS_LOCK_STALE_MS', 10 * 60_000),
  });
...
function parseCliArgs(argv) {
  ...
  if (opts.backfill) {
    ...
    if (positionals.length !== 1) {
      throw new SeedError('USAGE', 'Usage: node career-one.mjs followup-seed <appNum> [--date YYYY-MM-DD] [--force] [--dry-run] [--json]');
    }
