 ...
 *
 * Usage:
 *   node career-one.mjs set-status <report#|company> <state> [--note "..."] [--role "..."] [--dry-run] [--json]
 *
 * Row resolution:
...
const USAGE = `Usage: node career-one.mjs set-status <report#|company> <state> [--note "..."] [--role "..."] [--dry-run] [--json]
...
if (flags.json) {
  ...
  if (statusChanged && !flags.dryRun && newStatus === 'Applied') {
    console.error('ℹ️  Status is Applied — consider seeding follow-ups in data/follow-ups.md (#1430: node career-one.mjs followup-cadence)');
  }
}
