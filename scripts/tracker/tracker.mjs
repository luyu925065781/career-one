 ...
 *
 * Usage:
 *   node career-one.mjs tracker sync [--check]             # (re)build applications.db from applications.md
 *                                               # --check: diagnose only, no write; exit 1 if issues found
 *   node career-one.mjs tracker query [--status Applied] [--company acme] [--role designer]
 *                          [--since 2026-01-01] [--id N] [--limit 20] [--json]
 *   node career-one.mjs tracker history --id N             # status transition log observed across syncs
 *   node career-one.mjs tracker export [--out FILE]        # inverse: applications.db → canonical markdown (stdout by default)
 *   node career-one.mjs tracker delete --num N [--dry-run] # remove one application row from applications.md + reindex
 *
...
const MD_PATH = process.env.CAREER_ONE_TRACKER || 'data/applications.md';
const DB_PATH = process.env.CAREER_ONE_TRACKER_DB
  || (MD_PATH.endsWith('.md') ? MD_PATH.slice(0, -3) + '.db' : MD_PATH + '.db');

// SQLite must never open the source of truth itself (an explicit
// CAREER_ONE_TRACKER_DB could point both names at the same file).
...
function loadStates() {
  if (!existsSync(STATES_PATH)) {
    console.error(`Error: ${STATES_PATH} not found — cannot validate statuses. Run from the career-one root.`);
    process.exit(1);
  }
...
function reportDiagnostics(diag) {
  ...
  console.error('Fix at the source with `node career-one.mjs normalize` / `node career-one.mjs dedup`, then re-sync.');
  return total;
}
...
async function deleteApp(args) {
  ...
  if (!num) {
    console.error('Usage: node career-one.mjs tracker delete --num <N> [--dry-run]   (remove one application row by its number)');
    process.exit(1);
  }
  if (!existsSync(MD_PATH)) {
