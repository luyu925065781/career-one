 ...
 * ## Usage
 *
 *   node career-one.mjs reserve-report-num
...
const REPORTS_DIR = process.env.CAREER_ONE_REPORTS_DIR || join(__dirname, 'reports');
...
if (cmd === '--release') {
  ...
  if (!m) {
    process.stderr.write('Usage: node career-one.mjs reserve-report-num --release <NNN>[-<MMM>]\n');
    process.exit(1);
  }
...
if (cmd === '--count') {
  if (!/^\d+$/.test(arg || '')) {
    process.stderr.write(`Usage: node career-one.mjs reserve-report-num --count <1-${MAX_COUNT}>\n`);
    process.exit(1);
  }
