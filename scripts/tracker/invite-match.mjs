#!/usr/bin/env node
/**
 * invite-match.mjs — Interview-Invite → Tracker Matcher for career-one
 *
 ...
 *
 * Run: node career-one.mjs invite-match < invite.txt          (JSON to stdout)
 *      node career-one.mjs invite-match --file invite.txt
 *      echo "..." | node career-one.mjs invite-match --summary
 *      node career-one.mjs invite-match --self-test
 *
 * Issue #1495 — github.com/luyu925065781/career-one
 */
...
function printSummary(result) {
  console.log(`\n${'='.repeat(70)}`);
  console.log('  Interview Invite Matcher — career-one');
