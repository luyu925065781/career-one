 ...
 *
 * Usage:
 *   node career-one.mjs find <query> [--json]
 *
 ...
 * would be misleading when printed from the career-one root.
 *
...
function main() {
  ...
  if (!query) {
    console.log('Usage: node career-one.mjs find <report# | tracker# | company/role fragment> [--json]');
    process.exitCode = 1;
    return;
  }

  const trackerPath = process.env.CAREER_ONE_TRACKER || resolve(ROOT, 'data', 'applications.md');
