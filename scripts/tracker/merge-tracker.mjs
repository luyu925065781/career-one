 ...
 *
 * Run: node career-one.mjs merge [--dry-run] [--verify]
 */
...
import { roleFuzzyMatch } from '../scan/role-matcher.mjs';
...
// (original). CAREER_ONE_TRACKER overrides the path (used by tests and
...
const APPS_FILE = resolveTrackerPath(CAREER_ONE);
const TRACKER_DIR = dirname(APPS_FILE);
// CAREER_ONE_ADDITIONS overrides the additions dir (used by tests, mirrors CAREER_ONE_TRACKER).
const ADDITIONS_DIR = process.env.CAREER_ONE_ADDITIONS
  ? process.env.CAREER_ONE_ADDITIONS
  : join(CAREER_ONE, 'batch/tracker-additions');
const MERGED_DIR = join(ADDITIONS_DIR, 'merged');
const DRY_RUN = process.argv.includes('--dry-run');
const VERIFY = process.argv.includes('--verify');
const MIGRATE = process.argv.includes('--migrate');
const MIGRATE_VIA = process.argv.includes('--migrate-via');
const MERGE_HOLD_MS = Number(process.env.CAREER_ONE_MERGE_HOLD_MS) || 0;
const MERGE_READY_IPC = process.env.CAREER_ONE_MERGE_READY_IPC === '1';
 ...
 *
 * Used by the regression test hook (`CAREER_ONE_MERGE_HOLD_MS`), which
...
try {
  trackerLock = await acquireTrackerLock(TRACKER_LOCK_DIR, {
    timeoutMs: Number(process.env.CAREER_ONE_TRACKER_LOCK_TIMEOUT_MS) || 60_000,
    retryMs: Number(process.env.CAREER_ONE_TRACKER_LOCK_RETRY_MS) || 75,
    staleMs: Number(process.env.CAREER_ONE_TRACKER_LOCK_STALE_MS) || 10 * 60_000,
    tracker: APPS_FILE,
  });
...
/** A translated report is an independent tracker record, not a view of the
 * original. Explicit language tags keep same-company, same-role reports from
 * being collapsed by the fuzzy duplicate guard. */
function extractReportLanguage(notes) {
  const m = String(notes || '').match(/\[report-language:\s*([^\]]+)\]/i);
...
// to the tracker file's directory (see #760). Run with: node career-one.mjs merge --migrate
if (MIGRATE) {
  ...
  if (DRY_RUN) {
    ...
    console.log(`✅ Migration: rewrote ${changed} report link(s) in ${basename(APPS_FILE)} relative to ${TRACKER_DIR === CAREER_ONE ? 'repo root' : 'data/'}`);
  }
  process.exit(0);
}
...
// Run with: node career-one.mjs merge --migrate-via [--dry-run]
if (MIGRATE_VIA) {
...
for (const file of tsvFiles) {
  ...
  if (addition.via && COLMAP.via == null) {
    console.warn(`⚠️  ${file}: carries via=${addition.via} but the tracker has no Via column — value dropped. Add it with: node career-one.mjs merge --migrate-via`);
    addition.via = '';
  }
  ...
  if (!duplicate) {
    ...
    duplicate = existingApps.find(app => {
      ...
      const additionLanguage = extractReportLanguage(addition.notes);
      const existingLanguage = extractReportLanguage(app.notes);
      if (additionLanguage && existingLanguage && additionLanguage !== existingLanguage) return false;
...
if (VERIFY && !DRY_RUN) {
  ...
  try {
    execFileSync('node', [join(CAREER_ONE, 'scripts/system/verify-pipeline.mjs')], { stdio: 'inherit' });
  } catch (e) {
    process.exit(1);
  }
}
