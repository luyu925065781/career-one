 ...
 * next `/career-one pipeline` run both re-surface them, and they get evaluated
 ...
 *
 * Run: node career-one.mjs reconcile [--dry-run] [--state <path>] [--pipeline <path>]
 */
...
if (process.argv.includes('-h') || process.argv.includes('--help')) {
  console.log('Usage: node career-one.mjs reconcile [--dry-run] [--state <path>] [--pipeline <path>]');
...
function resolveInsideRepo(inputPath, fallbackPath, flag) {
  ...
  try {
    repoReal = realpathSync(CAREER_ONE);
...
const defaultPipeline = existsSync(join(CAREER_ONE, 'data/pipeline.md'))
  ? join(CAREER_ONE, 'data/pipeline.md')
  : join(CAREER_ONE, 'pipeline.md');
const PIPELINE_FILE = resolveInsideRepo(argValue('--pipeline'), defaultPipeline, '--pipeline');
const STATE_FILE = resolveInsideRepo(argValue('--state'), join(CAREER_ONE, 'batch/batch-state.tsv'), '--state');
const REPORTS_DIR = join(CAREER_ONE, 'reports');
...
for (let i = pendStart + 1; i < pendEnd; i++) {
  ...
  const reportLink = normalizeReportLink(`[${num}](reports/${reportFile})`, dirname(PIPELINE_FILE), CAREER_ONE);
