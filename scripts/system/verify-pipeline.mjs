#!/usr/bin/env node
/**
 * verify-pipeline.mjs — Health check for career-one pipeline integrity
 *
 * Checks:
 * 1. All statuses are canonical (per states.yml)
 * 2. No duplicate company+role+report-language entries
 ...
 * 9. No two report files cover the same company+role+language (warning — see #1425)
 * 10. Every report file has a tracker row referencing it (warning — see #1425)
 *
 * Run: node career-one.mjs verify
 */
...
// CAREER_ONE_TRACKER overrides the path (used by tests and non-standard layouts).
const APPS_FILE = process.env.CAREER_ONE_TRACKER
  ? process.env.CAREER_ONE_TRACKER
  : existsSync(join(CAREER_ONE, 'data/applications.md'))
...
let dupes = 0;
for (const e of entries) {
  const language = e.notes.match(/\[report-language:\s*([^\]]+)\]/i)?.[1]?.trim().toLowerCase() || 'unspecified';
  const key = e.company.toLowerCase().replace(/[^a-z0-9]/g, '') + '::' +
    e.role.toLowerCase().replace(/[^a-z0-9 ]/g, '') + '::' + language;
  if (!companyRoleMap.has(key)) companyRoleMap.set(key, []);
  companyRoleMap.get(key).push(e);
}
for (const [key, group] of companyRoleMap) {
  if (group.length > 1) {
    warn(`Possible duplicates: ${group.map(e => `#${e.num}`).join(', ')} (${group[0].company} — ${group[0].role}, same report language)`);
    dupes++;
  }
}
...
for (const e of entries) {
  ...
  if (!match) continue;
  const link = match[1];
  if (!existsSync(join(TRACKER_DIR, link)) && !existsSync(join(CAREER_ONE, link))) {
...
for (const name of reportFiles) {
  ...
  let role = null;
  let language = 'unspecified';
  try {
    const reportContent = readFileSync(join(REPORTS_DIR, name), 'utf-8');
    role = extractRole(reportContent);
    language = reportContent.match(/^\*\*Language:\*\*\s*(.+)$/mi)?.[1]?.trim().toLowerCase() || 'unspecified';
  } catch {
    // Unreadable report — the orphan check below still sees it.
  }
  if (!role) continue;
  const key = normalizeKey(companySlug) + '::' + normalizeKey(role) + '::' + language;
  if (!reportsByRole.has(key)) reportsByRole.set(key, []);
  reportsByRole.get(key).push(name);
}
for (const group of reportsByRole.values()) {
  if (group.length > 1) {
    warn(`Duplicate reports for same company+role+language: ${group.join(', ')}`);
    dupReports++;
  }
}
if (dupReports === 0) ok('No duplicate reports for the same company+role+language');
...
for (const e of entries) {
  ...
  if (company === '?') {
    if (COLMAP.via == null) {
      warn(`#${e.num}: unknown employer (?) but the tracker has no Via column — add it with: node career-one.mjs merge --migrate-via`);
      viaIssues++;
    } else if (!via || via === '—') {
