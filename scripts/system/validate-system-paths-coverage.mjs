...
// Include untracked release candidates so a local migration cannot pass by
// hiding new files from the coverage gate. Deleted cached paths are ignored.
const existing = tracked.filter((f) => existsSync(join(ROOT, f)));
const orphans = existing.filter((f) => !covered(f));
...
console.log(`OK: ${existing.length} working-tree files covered by SYSTEM_PATHS or USER_PATHS`);
process.exit(0);
