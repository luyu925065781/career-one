...
async function runSelfTest() {
  const tmp = mkdtempSync(join(tmpdir(), 'career-one-validate-portals-self-test-'));
  try {
...
async function main() {
  ...
  if (!filePath) {
    console.error('Usage: node career-one.mjs validate-portals [--file portals.yml] [--self-test]');
    process.exit(1);
  }
