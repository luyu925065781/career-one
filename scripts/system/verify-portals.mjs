 ...
 *
 * Usage:
 *   node career-one.mjs verify-portals                 # sweep tracked_companies in portals.yml
 *   node career-one.mjs verify-portals --add cursor    # probe slug variants for one name
 *   node career-one.mjs verify-portals --strict        # exit non-zero if any slug is unresolved
 *   node career-one.mjs verify-portals --file <path>   # use a specific portals file
 *
...
const DEFAULT_PORTALS_PATH = process.env.CAREER_ONE_PORTALS || 'portals.yml';
...
const PROVIDERS_DIR = resolve(dirname(dirname(dirname(fileURLToPath(import.meta.url)))), 'providers');
...
// Only run main() when invoked directly (`node career-one.mjs verify-portals`), not when
