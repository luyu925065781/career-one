...
const NAME_RE = /^career-one-plugin-([a-z0-9][a-z0-9-]*)$/;
...
export function parseRepoArg(arg) {
  ...
  if (!m) throw new Error(`repo must be named "career-one-plugin-<name>" (got "${repoName}")`);
  return { url, id: m[1] };
}
...
export function installFromRepo(root, { url, sha }) {
  ...
  if (existsSync(dest)) throw new Error(`plugins.local/${id} already exists — \`node career-one.mjs plugins remove ${id}\` first`);
