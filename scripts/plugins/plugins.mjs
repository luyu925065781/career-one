 ...
 *
 *   node career-one.mjs plugins list                       # discovered plugins + status
 *   node career-one.mjs plugins run <id> [hook] [args…]    # run one hook of one plugin
 *   node career-one.mjs plugins run gmail                  # ingest (the plugin's only hook)
 *   node career-one.mjs plugins run notion search "staff platform engineer"
 *   node career-one.mjs plugins run notion export [--dry-run]
 *
 * Provider plugins are NOT run here — they ride `node career-one.mjs scan` via a
 * `provider: <id>` entry in portals.yml. Keeping ingest/search/notify/export
 * behind this explicit CLI is deliberate: a plain `node career-one.mjs scan` never silently
...
async function cmdList() {
  ...
  console.log('\nEnable in config/plugins.yml, add keys to .env, then `node career-one.mjs plugins run <id>`.');
}
...
async function cmdRun(args) {
  ...
  if (!id) { console.error('Usage: node career-one.mjs plugins run <id> [hook] [args…] [--dry-run]'); process.exit(1); }

  const cfg = await loadPluginConfig(ROOT);
  const manifest = discoverPlugins(pluginRoots(ROOT), resolveSuccessorIds(ROOT)).find(m => m.id === id);
  if (!manifest) { console.error(`Unknown plugin "${id}". Run \`node career-one.mjs plugins list\`.`); process.exit(1); }
  ...
  if (!hook) {
    ...
    else { console.error(`Plugin "${id}" exposes multiple hooks (${runnable.join(', ')}). Specify one: node career-one.mjs plugins run ${id} <hook>`); process.exit(1); }
  }
  ...
  if (hook === 'ingest' || hook === 'search') {
    ...
    if (hook === 'search' && !payload) { console.error(`search needs a query: node career-one.mjs plugins run ${id} search "<query>"`); process.exit(1); }
    ...
    if (jobs.length) { appendToPipeline(jobs); console.log(`→ Appended ${jobs.length} to data/pipeline.md. Run /career-one pipeline to evaluate.`); }
    return;
  }
  ...
  if (hook === 'notify') {
    const message = positional.slice(hookArgStart).join(' ') || '(career-one notification)';
...
function setEnabled(id, on, settings) {
  ...
  writeFileSync(file, '# career-one plugin activation — see config/plugins.example.yml\n' + yaml.dump(cfg), 'utf8');
}
...
async function cmdAvailable() {
  ...
  if (reg.plugins.length === 0) {
    ...
    console.log('\nInstall:  node career-one.mjs plugins add <name>');
  }
}
...
function cmdSkill(args) {
  ...
  if (!id) {
    ...
    console.log(withSkill.length ? 'Plugins that ship a skill:\n' + withSkill.map(m => `  ${m.id}`).join('\n') + '\n\nRead one: node career-one.mjs plugins skill <id>' : 'No installed plugin ships a skill.');
    return;
  }
...
function cmdEnable(args) {
  ...
  if (!id) { console.error('Usage: node career-one.mjs plugins enable <id> [--confirm]'); process.exit(1); }
  const m = findManifest(id);
  if (!m) { console.error(`Unknown plugin "${id}". Run \`node career-one.mjs plugins list\`.`); process.exit(1); }
  const lock = readLock(ROOT);
  const entry = lock.plugins?.[id];
  const source = classifySource(m, ROOT, entry);
  if (!confirm) {
    console.log(capabilityCard(m, source));
    if (source === 'unverified') console.log('\n  ⚠️  UNVERIFIED — not reviewed by career-one; you are trusting this author.');
    console.log(`\n  This grants the capabilities above. To confirm, run:\n    node career-one.mjs plugins enable ${id} --confirm\n`);
    return;
  }
...
function cmdRemove(args) {
  const id = args[0];
  if (!id) { console.error('Usage: node career-one.mjs plugins remove <id>'); process.exit(1); }
...
function cmdNew(args) {
  const name = args[0];
  if (!name) { console.error('Usage: node career-one.mjs plugins new <name>'); process.exit(1); }
  let dest;
...
async function cmdAdd(args) {
  ...
  const confirm = args.includes('--confirm');
  if (!target) { console.error('Usage: node career-one.mjs plugins add <name|owner/repo> [--sha <commit>] [--confirm]'); process.exit(1); }
  ...
  if (reg && !target.includes('/')) {
    ...
    console.log("⚠️  Not in the career-one registry — you are installing this author's code with your privileges.");
  }
  ...
  else console.log(`\n  To enable it (grants the above), run:  node career-one.mjs plugins enable ${installed.id} --confirm`);
}
...
async function main() {
  ...
  switch (cmd) {
    ...
    default:
      console.error('Usage: node career-one.mjs plugins [list | available | run <id> [hook] | skill <id> | new <name> | add <name|owner/repo> [--sha <c>] [--confirm] | enable <id> [--confirm] | trust <id> | remove <id>]');
      process.exit(1);
  }
}
