 ...
 *
 * Usage:
 *   node career-one.mjs pdf <input.html> <output.pdf> [--format=letter|a4] [--report=NNN] [--allow-reorder]
 *
 ...
 * or outside the career-one repository.
 *
...
export function injectPrintPageCss(html, format = 'a4') {
  ...
  const pageStyle = `<style id="career-one-page-setup">\n@page { size: ${pageSize}; margin: ${PDF_PAGE_MARGIN}; }\n</style>`;
 ...
 * career-one root with forward slashes. One row per PDF path; when a report
...
async function generatePDF() {
  ...
  if (!inputPath || !outputPath) {
    console.error('Usage: node career-one.mjs pdf <input.html> <output.pdf> [--format=letter|a4] [--report=NNN] [--allow-reorder]');
    console.error('');
    ...
    console.error('mechanical markdown-to-HTML step by design. Run `/career-one pdf` in your AI');
...
export async function renderHtmlToPdf(html, outputPath, opts = {}) {
  ...
  const tmpHtmlPath = resolve(baseDir, `.career-one-render-${randomUUID()}.html`);
