...
async function main() {
  ...
  if (args.length === 0 || args.includes('--help')) {
    console.error('Usage:');
    console.error('  node career-one.mjs build-cv-latex <input.json> <output.tex>');
    console.error('  node career-one.mjs build-cv-latex --test');
    process.exit(1);
  }
  ...
  if (!inputPath || !outputPath) {
    console.error('Usage: node career-one.mjs build-cv-latex <input.json> <output.tex>');
    process.exit(1);
  }
