...
async function callOpenRouter(systemPrompt, userMessage) {
  ...
  const pinnedModel = process.env.CAREER_ONE_MODEL;
  if (pinnedModel) {
    ...
    try {
      const resp = await fetch(OPENROUTER_API_URL, {
        ...
        headers: {
          ...
          'HTTP-Referer':  'https://github.com/luyu925065781/career-one',
          'X-Title':       'career-one',
        },
        body,
        signal: ctrl.signal,
...
async function fetchJobPage(url) {
  ...
  try {
    const r = await fetch(url, {
      headers: { 'User-Agent': 'Mozilla/5.0 (compatible; career-one/1.0)' }
    });
...
// search-query companies are handled by the full /career-one scan pipeline.
...
async function cmdScan() {
  ...
  if (added > 0) {
    console.log('\n→  node career-one.mjs openrouter pipeline\n   to evaluate pending listings.\n');
  }
}
...
// Entry point
// ---------------------------------------------------------------------------
// Only run the CLI when invoked directly (`node career-one.mjs openrouter ...`), so the
...
if (invokedDirectly && ['evaluate', 'eval', 'pipeline', 'apply', 'models'].includes(command) && !process.env.CAREER_ONE_MODEL) {
...
if (invokedDirectly) switch (command) {
  ...
  case 'apply':
    if (!args[0]) { console.error('Usage: node career-one.mjs openrouter apply <report_num|report_path>'); break; }
...
COMMANDS:
  node career-one.mjs openrouter scan              → Scan Greenhouse APIs for new matching listings
  node career-one.mjs openrouter evaluate <url>    → Evaluate a listing by URL
  node career-one.mjs openrouter evaluate          → Paste a job description interactively
  node career-one.mjs openrouter pipeline          → Batch-evaluate all pending entries in pipeline.md
  node career-one.mjs openrouter apply <report_no> → Generate application form answers from a report
  node career-one.mjs openrouter models            → List available free models from OpenRouter
...
MODEL SELECTION:
  ...
  - Pin a model:  CAREER_ONE_MODEL=deepseek/deepseek-r1:free node career-one.mjs openrouter eval <url>
`);
}
