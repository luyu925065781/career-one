#!/usr/bin/env node
/**
 * openai-eval.mjs — OpenAI-compatible Job Offer Evaluator for career-one
 *
 ...
 *
 * Usage:
 *   node career-one.mjs openai "Paste full JD text here"
 *   node career-one.mjs openai --file ./jds/my-job.txt
 *   node career-one.mjs openai --url https://openrouter.ai/api/v1 --model meta-llama/llama-3.3-70b-instruct --file ./jds/job.txt
 *
...
if (args.length === 0 || args[0] === '--help' || args[0] === '-h') {
  console.log(`
╔══════════════════════════════════════════════════════════════════╗
║       career-one — OpenAI-compatible Evaluator (any endpoint)     ║
╚══════════════════════════════════════════════════════════════════╝

  Evaluate a job offer with any OpenAI-compatible chat API instead of Claude.

  USAGE
    node career-one.mjs openai "<JD text>"
    node career-one.mjs openai --file ./jds/my-job.txt
    node career-one.mjs openai --url <base> --model <id> --file ./jds/job.txt
  ...
  EXAMPLES
    OPENAI_API_KEY=sk-... node career-one.mjs openai --file ./jds/job.txt
    node career-one.mjs openai --url http://localhost:1234/v1 --model local "<JD text>"
`);
  process.exit(0);
}
...
❌  No API key for ${endpointHost}.
   ...
   Set one and re-run:
     OPENAI_API_KEY=your_key node career-one.mjs openai ...
...
// ---------------------------------------------------------------------------
console.log('\n' + '═'.repeat(66));
console.log('  CAREER-ONE EVALUATION — powered by ' + modelName + ' (' + endpointHost + ')');
