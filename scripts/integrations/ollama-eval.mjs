#!/usr/bin/env node
/**
 * ollama-eval.mjs — Ollama-powered Job Offer Evaluator for career-one
 *
 ...
 *
 * Usage:
 *   node career-one.mjs ollama "Paste full JD text here"
 *   node career-one.mjs ollama --file ./jds/my-job.txt
 *   node career-one.mjs ollama --model qwen2.5:72b --file ./jds/my-job.txt
 *
 * Requires:
...
if (args.length === 0 || args[0] === '--help' || args[0] === '-h') {
  console.log(`
╔══════════════════════════════════════════════════════════════════╗
║           career-one — Ollama Evaluator (local / free)          ║
╚══════════════════════════════════════════════════════════════════╝

  Evaluate a job offer using a local Ollama model instead of Claude.

  USAGE
    node career-one.mjs ollama "<JD text>"
    node career-one.mjs ollama --file ./jds/my-job.txt
    node career-one.mjs ollama --model qwen2.5:72b "<JD text>"
  ...
  EXAMPLES
    node career-one.mjs ollama "We are looking for a Senior AI Engineer..."
    node career-one.mjs ollama --file ./jds/openai-swe.txt
    OLLAMA_MODEL=mistral-nemo node career-one.mjs ollama --file ./jds/job.txt
`);
  process.exit(0);
}
...
❌  Remote Ollama endpoint detected: ${baseUrl}
   ...
   Ollama on a home server), set:
     OLLAMA_ALLOW_REMOTE=1 node career-one.mjs ollama ...
`);
    process.exit(1);
  }
}
...
// ---------------------------------------------------------------------------
console.log('\n' + '═'.repeat(66));
console.log('  CAREER-ONE EVALUATION — powered by Ollama (' + modelName + ')');
