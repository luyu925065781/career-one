#!/usr/bin/env node
/**
 * gemini-eval.mjs — Gemini-powered Job Offer Evaluator for career-one
 *
 ...
 *
 * Usage:
 *   node career-one.mjs gemini "Paste full JD text here"
 *   node career-one.mjs gemini --file ./jds/my-job.txt
 *
 * Requires:
...
const PATHS = {
  ...
  evaluate:    join(ROOT, '.claude', 'skills', 'career-one', 'SKILL.md'),
  cv:          join(ROOT, 'cv.md'),
...
if (args.length === 0 || args[0] === '--help' || args[0] === '-h') {
  console.log(`
╔══════════════════════════════════════════════════════════════════╗
║           career-one — Gemini Evaluator (free-tier)             ║
╚══════════════════════════════════════════════════════════════════╝

  Evaluate a job offer using Google Gemini instead of Claude.

  USAGE
    node career-one.mjs gemini "<JD text>"
    node career-one.mjs gemini --file ./jds/my-job.txt
    node career-one.mjs gemini --model gemini-2.5-flash "<JD text>"
  ...
  EXAMPLES
    node career-one.mjs gemini "We are looking for a Senior AI Engineer..."
    node career-one.mjs gemini --file ./jds/openai-swe.txt
`);
  process.exit(0);
}
...
function validateEvaluationShape(text) {
  ...
  if (issues.length > 0) {
    throw new Error(`Gemini returned an invalid career-one report: ${issues.join('; ')}`);
  }
}
...
// ---------------------------------------------------------------------------
const systemPrompt = `You are career-one, an AI-powered job search assistant.
...
// ---------------------------------------------------------------------------
console.log('\n' + '═'.repeat(66));
console.log('  CAREER-ONE EVALUATION — powered by Google Gemini');
...
`;
  ...
  if (reportSaved) {
    try {
      const mergeOutput = execFileSync(process.execPath, [join(ROOT, 'scripts/tracker/merge-tracker.mjs')], {
        cwd: ROOT,
        encoding: 'utf-8',
