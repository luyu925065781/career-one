...
function cliHelp() {
  return `择程AI共享任务协议\n\n` +
    `  node career-one.mjs run start --intent <意图> --title <标题> [--source agent|web]\n` +
    `  node career-one.mjs run queue --intent <意图> --title <标题> [--source web] [--instruction <续接指令>]\n` +
    `  node career-one.mjs run progress <run-id> --label <进度>\n` +
    `  node career-one.mjs run attach <run-id> --artifact path|label|/page [--label <进度>]\n` +
    `  node career-one.mjs run wait <run-id> --question <需要用户回答的问题> [--label <进度>]\n` +
    `  node career-one.mjs run complete <run-id> [--summary <摘要>] [--artifact path|label|/page]\n` +
    `  node career-one.mjs run fail <run-id> --error <原因>\n` +
    `  node career-one.mjs run list | get <run-id> | archive <run-id>\n` +
    `  node career-one.mjs run propose <run-id> --target <用户文件> --draft <草稿文件> --summary <摘要>\n` +
    `  node career-one.mjs run proposal <proposal-id> | approve <proposal-id> | reject <proposal-id>\n`;
}
