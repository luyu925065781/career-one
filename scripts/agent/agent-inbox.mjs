 ...
 * *acting on* it.
 *
 * career-one is driven from an AI session, but there's no durable place to drop
 ...
 *   node career-one.mjs agent-inbox list [--all]                 # pending only, or every item
 *   node career-one.mjs agent-inbox resolve 1 [--result "scored 4.3 — report 012"]
 *   node career-one.mjs agent-inbox resolve --task run-id [--result "output/cv.pdf"]
 */

import { readFileSync, writeFileSync, existsSync, mkdirSync } from 'fs';
import { dirname } from 'path';

const PATH = process.env.CAREER_ONE_INBOX || 'data/agent-inbox.md';

const HEADER = [
  '# Agent Inbox',
  '',
  '> **Agent protocol:** at the start of a career-one session, read this file.',
...
function ensureGitignored() {
  ...
  if (process.env.CAREER_ONE_INBOX || PATH !== 'data/agent-inbox.md') return;
  try {
...
function add() {
  ...
  if (!text) fail('add needs a request, e.g. node career-one.mjs agent-inbox add "evaluate https://..."');
  ensureFile();
...
if (cmd === 'add') add();
  ...
  process.stdout.write(
    'Usage:\n' +
    '  node career-one.mjs agent-inbox add "evaluate https://acme.com/jobs/42"\n' +
    '  node career-one.mjs agent-inbox list [--all]\n' +
    '  node career-one.mjs agent-inbox resolve <n> [--result "..."]\n' +
    '  node career-one.mjs agent-inbox resolve --task <run-id> [--result "..."]\n',
  );
}
