...
function verify() {
  ...
  const statusCode = run('verify-pipeline.mjs');
  const pending = pendingTsvs();
  if (pending.length > 0) {
...
function merge(args) {
  const statusCode = run('merge-tracker.mjs', args);
...
function find(args) {
  ...
  return run('find.mjs', args);
}

function help() {
  console.log(`Usage: node market-cn.mjs <command>
