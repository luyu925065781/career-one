 ...
 * `applications.md` (original root layout). The `CAREER_ONE_TRACKER` env var
 ...
 * on one spelling.
 *
 * @param {string} rootDir - The career-one repository root.
 * @returns {string} Absolute canonical tracker path.
 */
export function resolveTrackerPath(rootDir) {
  const raw = process.env.CAREER_ONE_TRACKER
 ...
 * contends on the same lock. `CAREER_ONE_TRACKER_LOCK` exists for tests and
 ...
 * use the career-one lock-name prefix. Invalid values are ignored and the
...
export function trackerLockDirFor(appsFile) {
  ...
  const fallback = join(tmpRoot, `career-one-merge-tracker-${lockKey}.lock`);
  const envValue = process.env.CAREER_ONE_TRACKER_LOCK;
  ...
  if (!basename(candidate).startsWith('career-one-merge-tracker-')) return fallback;
  return candidate;
}
